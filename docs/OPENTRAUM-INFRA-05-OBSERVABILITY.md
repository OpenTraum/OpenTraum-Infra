...
1. [개요](#1-개요)
1.5. [관측 스택 입문: 이 도구들이 무엇이고 왜 같이 쓰는가](#15-관측-스택-입문-이-도구들이-무엇이고-왜-같이-쓰는가)
2. [도구 스택과 버전](#2-도구-스택과-버전)
3. [3-pillar 흐름](#3-3-pillar-흐름)
...
8. [Tempo monolithic 매니페스트 라인 분석](#8-tempo-monolithic-매니페스트-라인-분석)
9. [Alloy DaemonSet과 OTLP 흐름](#9-alloy-daemonset과-otlp-흐름)
10. [Grafana datasource sidecar 자동 등록](#10-grafana-datasource-sidecar-자동-등록)
11. [분산 정책 (현재)](#11-분산-정책-현재)
12. [정량 / 튜닝 (현재)](#12-정량--튜닝-현재)
13. [트러블슈팅](#13-트러블슈팅)
14. [진단 명령어](#14-진단-명령어)
...
본 문서는 OpenTraum 클러스터 monitoring 네임스페이스의 관측 스택을 설명합니다. 관측은 metrics, logs, traces 세 축으로 구성되어 있고 모든 데이터는 Grafana 한 곳에서 조회됩니다.
...
- **traces**: 애플리케이션이 OpenTelemetry SDK 로 Alloy OTLP receiver 에 전송하고, Alloy 가 Tempo 로 forward 하여 PVC 에 저장합니다.

본문 사실 출처는 다음과 같습니다.

- 라이브 클러스터 상태: 2026-04-28 15:10 KST 기준 `kubectl get pod/svc/pvc -n monitoring` 및 `helm list -n monitoring` 결과.
- 매니페스트와 values 파일:
  - `k8s/monitoring/values-kube-prometheus-stack.yaml`
  - `k8s/monitoring/values-loki.yaml`
  - `k8s/monitoring/README.md`
  - `k8s/tempo/statefulset.yaml`, `configmap.yaml`, `service.yaml`
  - `k8s/alloy/configmap-patch.yaml`, `service-patch.yaml`
  - `k8s/grafana/datasource-tempo.yaml`

---

## 1.5 관측 스택 입문: 이 도구들이 무엇이고 왜 같이 쓰는가

본 문서가 다루는 Prometheus, Grafana, Loki, Tempo, Alloy, Alertmanager 는 각각 역할이 다른 컴포넌트입니다. 2장 이후 매니페스트를 라인 단위로 보기 전에, 이 절에서는 각 도구가 무엇이고 왜 같이 쓰이는지부터 정리합니다.

### 1.5.1 왜 관측이 3-pillar (metrics / logs / traces) 인가

분산 시스템에서 한 종류의 데이터만으로는 진단이 어렵습니다. CPU 사용률 같은 숫자 (metrics) 는 "지금 시스템이 어떤 상태인가" 를 빠르게 보여주지만 "왜 그 상태가 됐는지" 는 답하지 못합니다. 로그 (logs) 는 그 시점에 무슨 일이 있었는지 텍스트로 기록하지만 정량 분석이 어렵고 검색 비용이 큽니다. 트레이스 (traces) 는 한 요청이 여러 서비스를 어떻게 거쳤는지 보여주지만 모든 요청을 추적하면 비용이 폭발해 sampling 이 필요합니다.

→ 그래서 세 데이터를 함께 쌓고, 같은 식별자 (trace_id, service.name 등) 로 서로를 점프하게 만드는 것이 표준 패턴입니다. metrics 로 이상을 감지하고, logs 로 무슨 일이 있었는지 확인하고, traces 로 어느 호출이 느렸는지 좁혀가는 흐름입니다.
...
- **LogQL 예시**: `{namespace="opentraum", app="gateway"} |= "ERROR"` 는 monitoring 네임스페이스의 gateway 앱 로그 중 본문에 ERROR 가 들어간 라인입니다. 앞쪽 `{}` 부분이 라벨 필터 (인덱스 사용), 뒤쪽 `|=` 가 본문 필터 (스캔) 입니다.
...
- **정의**: 한 요청이 여러 서비스를 거치며 남기는 호출 그래프입니다. 각 호출 단위가 span 이고, 같은 요청에서 발생한 span 들은 동일한 `trace_id` 로 묶입니다.
- **왜 필요한가**: gateway → auth → user → reservation → payment 처럼 SAGA 식 분산 흐름에서 어디서 시간이 소요됐는지, 어디서 실패했는지를 한 화면에 펼쳐 봐야 합니다. 로그만 보면 요청이 어느 서비스를 거쳤는지 재구성하기 어렵습니다.
- **수집 경로**: 애플리케이션이 OpenTelemetry SDK 로 span 을 생성하고 OTLP (OpenTelemetry Protocol) 로 수집기에 전송합니다. 본 시스템에서는 앱 → Alloy OTLP receiver → Tempo distributor 순입니다.
- **Tempo monolithic mode**: distributor / ingester / querier / compactor 를 단일 Pod 에서 실행합니다. 학습 환경에 적합합니다.
- **Tempo 의 차별점**: trace 본문에 별도 인덱스를 만들지 않고 `trace_id` 로 직접 조회합니다. 검색 기능은 약한 대신 저장 비용이 작습니다. → 그래서 "임의 조건으로 trace 검색" 보다는 "metrics / logs 에서 얻은 trace_id 로 점프" 패턴이 표준입니다.
- **tracesToLogsV2**: Grafana 의 Tempo 데이터소스에서 trace span 의 `service.name` resource attribute 를 Loki 라벨 `service_name` 으로 자동 매핑하는 옵션입니다. 한 span 을 클릭하면 같은 서비스 + 같은 trace_id 의 Loki 로그로 바로 이동할 수 있습니다.

### 1.5.5 Collection Agent (Alloy)

- **정의**: Grafana 가 만든 OpenTelemetry 호환 수집 에이전트입니다. 옛 Grafana Agent 의 후속으로, 하나의 바이너리로 metrics / logs / traces 수집을 모두 다룹니다.
- **본 시스템에서 두 역할**:
  1. Pod stdout 을 수집해 loki-gateway 로 push 합니다 (`loki.source.kubernetes` + `loki.write`).
  2. 앱이 보내는 OTLP trace 를 받아 Tempo distributor 로 forward 합니다 (`otelcol.receiver.otlp` → `otelcol.exporter.otlphttp`).
...
### 1.5.6 Visualization (Grafana)

- **정의**: 등록된 데이터소스에 쿼리를 날려 대시보드 / 패널로 시각화하는 UI 입니다. 자체 저장소는 없고 backend 에 위임합니다.
- **본 시스템에서 데이터소스 3개**: Prometheus (default), Loki, Tempo. 한 화면에서 PromQL 패널 옆에 LogQL 패널을 두고, 패널 간 변수 (예: `pod`) 를 공유해 같은 시각의 metrics / logs 를 같이 볼 수 있습니다.
- **데이터소스 자동 등록 메커니즘**: Grafana Pod 옆에 sidecar 컨테이너 (`kiwigrid/k8s-sidecar`) 가 떠서 monitoring 네임스페이스의 ConfigMap 중 라벨 `grafana_datasource: "1"` 이 붙은 것을 watch 합니다. 매칭되는 ConfigMap 의 데이터를 Grafana provisioning 디렉토리로 마운트하거나 Grafana HTTP API 로 주입합니다. → 그래서 새 데이터소스를 추가할 때 Grafana 재기동 없이 ConfigMap apply 한 번으로 끝납니다. 수동 등록 작업은 0 입니다.
- **대시보드도 같은 패턴**: ConfigMap 라벨 `grafana_dashboard: "1"` 으로 자동 등록 가능합니다. 본 시스템은 현재 chart 가 기본 제공하는 대시보드 위주로 사용 중입니다.

### 1.5.7 Alerting (Alertmanager)
...
flowchart LR
    ...
    Loki[(Loki<br/>:3100)]
    TempoDist[Tempo distributor<br/>:4318]
    Tempo[(Tempo query<br/>:3200)]
    ...
    %% traces
    Gateway -- "OTLP HTTP 4318" --> Alloy
    Auth -- "OTLP HTTP 4318" --> Alloy
    Reservation -- "OTLP HTTP 4318" --> Alloy
    Payment -- "OTLP HTTP 4318" --> Alloy
    Alloy -- "OTLP HTTP 4318" --> TempoDist
    TempoDist --> Tempo
    ...
    Grafana -- "datasource HTTP :3200" --> Tempo
    User -- "HTTPS" --> Grafana

    class Gateway,Auth,UserSvc,Event,Reservation,Payment,Web app
    class Node,KSM,NodeExp app
    class Alloy,LokiGW,Loki,TempoDist,Tempo,Prom,Grafana obs
    class User user
```

3-pillar 가 모두 Alloy 또는 Prometheus 를 거쳐 각자의 backend 로 들어가고, Grafana 한 곳이 셋 모두를 데이터소스로 들고 있는 구조입니다. 이 그림은 3장의 더 정밀한 데이터 경로 다이어그램과 일부 겹치지만, 1.5 단계에서는 "어떤 도구가 어디 붙어 있나" 를 먼저 잡는 용도로 봅니다.

### 1.5.9 비교 표: Metrics vs Logs vs Traces

| 측면 | Metrics | Logs | Traces |
|---|---|---|---|
| 데이터 형태 | 시계열 숫자 | 시계열 텍스트 | DAG (span 그래프) |
| 인덱싱 | 라벨 기반 | 라벨만 (Loki) / 본문 (ELK) | trace_id |
| 도구 | Prometheus | Loki | Tempo |
| 저장 비용 | 카디널리티 의존 | 보존 기간 + 본문 크기 | trace 양 + 보존 기간 |
| 진단 강점 | "지금 시스템 상태?" | "그 시점 무슨 일?" | "왜 그 요청만 느렸나?" |
| 약점 | "왜?" 답 못함 | 검색 비용, 정량 분석 어려움 | 모든 요청 trace 시 비용 폭증 (sampling 필요) |

세 데이터는 강점과 약점이 정확히 보완 관계에 있어 함께 쌓을 때 비로소 분산 시스템 진단이 가능해집니다.

### 1.5.10 OpenTelemetry 가 표준이 된 이유

- **정의**: metrics / logs / traces 세 축 모두 다루는 벤더 중립 SDK + 프로토콜 (OTLP) 입니다. CNCF 산하 프로젝트로, 옛 OpenTracing + OpenCensus 가 합쳐져 만들어졌습니다.
- **본 시스템에서의 사용 범위**: traces 만 OpenTelemetry SDK 를 직접 사용합니다 (앱 → Alloy OTLP receiver → Tempo). metrics 는 Prometheus 의 `/metrics` 엔드포인트 (OpenMetrics, OTel 과 호환) 로 위임하고, logs 는 Pod stdout 으로 위임해 Alloy 가 수집합니다.
- **벤더 중립의 의미**: 한 번 OTLP 로 export 하면 backend 를 Tempo, Jaeger, Honeycomb, Datadog 등 무엇으로 바꿔도 앱 코드 수정 없이 수집기 (Alloy) 의 exporter 한 줄만 바꾸면 됩니다. → 그래서 trace backend 교체 비용이 거의 0 에 수렴하고, 이 점이 OpenTelemetry 가 사실상 표준이 된 가장 큰 이유입니다.
...
| `alloy` | 1.7.0 | v1.15.0 | 로그 수집 + OTLP trace 중계 (DaemonSet) |
...
| `alloy` | 12345 |
| `alloy-otlp` | 4317 (otlp-grpc), 4318 (otlp-http) |
...
| `tempo` | 3200, 9095, 4317, 4318 |

PVC 라이브 인벤토리는 다음과 같습니다.

| PVC | 용량 | Access | StorageClass |
|---|---|---|---|
| `data-tempo-0` | 10Gi | RWO | `ebs-sc` |
| `storage-loki-0` | 10Gi | RWO | `ebs-sc` |

DaemonSet 인 alloy 와 loki-canary 가 7개 노드 중 6개에서 기동된 이유는 g5.xlarge 한 노드에 taint 가 있어 일반 DaemonSet 이 회피하기 때문입니다. 본문에서는 "노드당 1 Pod (DaemonSet)" 으로 표기하고 라이브 시점 실제 수는 6 입니다.

---

## 3. 3-pillar 흐름

세 축의 데이터 경로를 한 다이어그램으로 표현하면 다음과 같습니다.
...
flowchart LR
    ...
    %% Traces
    App -- "OTLP gRPC :4317 / HTTP :4318" --> Alloy
    Alloy -- "OTLP HTTP :4318" --> Tempo
    Tempo --> TempoPVC

    %% Read path
    Prom -- "PromQL" --> Grafana
    Loki -- "LogQL" --> Grafana
    Tempo -- "TraceQL :3200" --> Grafana
    User -- "HTTPS" --> Grafana

    class User user
    class Grafana obs
    class Alloy,SM obs
    class Prom,Loki,LokiGW,Tempo obs
    class TempoPVC,LokiPVC data
    class App,Node app
```

각 화살표 라벨에 포트와 프로토콜을 명시했습니다. metrics 는 HTTP scrape, logs 는 HTTP push, traces 는 OTLP (gRPC 4317 또는 HTTP 4318) 로 입수되고 Grafana 가 세 backend 모두 데이터소스로 보유합니다.
...
- **podMetadata.labels**: `opentraum.io/heavy: "true"`. 다른 무거운 컴포넌트 (Loki, MariaDB, Tempo) 와 cross podAntiAffinity 를 매칭하기 위한 공용 라벨입니다.
...
`opentraum.io/heavy=true` 라벨은 Prometheus, Loki, MariaDB, Tempo 4개 무거운 Pod 가 공통으로 들고 있어, 4개 Pod 가 서로 다른 노드를 선호하게 만드는 단일 식별자 역할을 합니다.
...
데이터소스 자동 등록 흐름은 다음과 같이 동작합니다. sidecar 컨테이너 (`kiwigrid/k8s-sidecar:2.7.1`) 가 monitoring 네임스페이스의 ConfigMap 중 라벨 `grafana_datasource: "1"` 이 붙은 것을 watch 하다가, 매칭되는 ConfigMap 의 데이터를 Grafana provisioning 디렉토리로 마운트하거나 Grafana HTTP API 로 주입합니다. 새 ConfigMap 을 apply 하면 Grafana 재기동 없이 데이터소스가 추가됩니다. Tempo 는 별도 ConfigMap (`grafana-datasource-tempo`) 으로 등록되고, Loki 는 values 의 `additionalDataSources` 로 인라인 등록되는 두 가지 경로가 함께 사용됩니다.
...
## 8. Tempo monolithic 매니페스트 라인 분석

Tempo 는 Helm chart 가 아닌 자체 매니페스트 (`k8s/tempo/`) 로 배포되어 있고 단일 StatefulSet 에서 distributor, ingester, compactor, querier, query-frontend 를 모두 실행합니다.

`statefulset.yaml` 핵심:

- **kind / replicas**: `StatefulSet`, `replicas: 1`. monolithic 모드.
- **image**: `grafana/tempo:2.6.0` (Docker Hub 직접, Harbor 미사용).
- **ports**: `http 3200` (Tempo query API), `grpc 9095` (internal), `otlp-grpc 4317`, `otlp-http 4318`.
- **volumeMounts**: `/etc/tempo` (ConfigMap), `/var/tempo` (PVC).
- **volumeClaimTemplates**: `data` 10Gi, `RWO`, `storageClassName=ebs-sc`. 라이브 PVC 는 `data-tempo-0`.
- **securityContext**: `fsGroup=10001`, `runAsUser=10001`, `runAsGroup=10001`, `runAsNonRoot=true`. 비특권 컨테이너.
- **resources**: requests `cpu 100m / mem 256Mi`, limits `cpu 500m / mem 512Mi`.
- **readinessProbe**: HTTP `GET /ready` on `http`, initialDelay 20s, period 10s, timeout 3s.
- **livenessProbe**: HTTP `GET /ready` on `http`, initialDelay 60s, period 30s, timeout 5s.
- **topologySpreadConstraints**: `maxSkew=1`, `ScheduleAnyway`, `labelSelector=app.kubernetes.io/part-of=monitoring`. monitoring 라벨 단위로 노드 분산.

`configmap.yaml` 의 Tempo config 핵심:

- **server**: `http_listen_port: 3200`, `grpc_listen_port: 9095`.
- **distributor.receivers.otlp.protocols**: `http=0.0.0.0:4318`, `grpc=0.0.0.0:4317`. Alloy 가 보내는 OTLP 두 프로토콜 모두 수신.
- **ingester**: `trace_idle_period=10s`, `max_block_bytes=1_000_000`, `max_block_duration=5m`.
- **compactor.compaction**: `compaction_window=1h`, `block_retention=48h`. trace 는 48시간 보관.
- **storage.trace.backend**: `local`, `local.path=/var/tempo/traces`, `wal.path=/var/tempo/wal`.

`service.yaml` 은 ClusterIP 로 4 포트 (`3200`, `9095`, `4317`, `4318`) 를 동시 노출합니다. Grafana 는 3200 으로 query 하고 Alloy 는 4318 (OTLP HTTP) 로 export 합니다.

---

## 9. Alloy DaemonSet과 OTLP 흐름

Alloy 는 노드당 1 Pod 의 DaemonSet 입니다. `configmap-patch.yaml` 은 chart 가 만든 기본 alloy ConfigMap 을 통째로 덮어쓰는 patch 매니페스트로, 두 가지 파이프라인을 정의합니다.

### 9.1 Loki push 파이프라인 (기존)
...
receiver 는 0.0.0.0 에 binding 되어 4317 (gRPC) 와 4318 (HTTP) 둘 다 수신하고, output 의 traces 를 Tempo exporter 로 forward 합니다. exporter 는 클러스터 내부 통신이므로 TLS 비활성으로 4318 HTTP 로 push 합니다.

`service-patch.yaml` 은 chart 의 기본 `alloy` Service 와 별도로 `alloy-otlp` ClusterIP Service 를 추가합니다. selector 는 `app.kubernetes.io/name=alloy` + `instance=alloy` 로 chart Pod 를 그대로 가리키고 4317 / 4318 두 포트만 노출합니다. 애플리케이션은 `alloy-otlp.monitoring:4317` (gRPC) 또는 `alloy-otlp.monitoring:4318` (HTTP) 로 trace 를 보냅니다.
...
flowchart LR
    ...
    AppPod[App Pod<br/>OpenTelemetry SDK]
    SvcOTLP[alloy-otlp Service<br/>ClusterIP 4317/4318]
    AlloyDS[Alloy DaemonSet Pod<br/>otelcol.receiver.otlp]
    AlloyExp[otelcol.exporter.otlphttp.tempo]
    TempoSvc[tempo Service<br/>4318]
    TempoPod[Tempo StatefulSet Pod<br/>distributor + ingester]
    TempoPVC[(/var/tempo PVC 10Gi)]

    AppPod -- "OTLP gRPC :4317<br/>또는 HTTP :4318" --> SvcOTLP
    SvcOTLP --> AlloyDS
    AlloyDS --> AlloyExp
    AlloyExp -- "OTLP HTTP :4318<br/>tls insecure" --> TempoSvc
    TempoSvc --> TempoPod
    TempoPod -- "block + WAL" --> TempoPVC

    class AppPod app
    class SvcOTLP,AlloyDS,AlloyExp,TempoSvc,TempoPod obs
    class TempoPVC data
```

---

## 10. Grafana datasource sidecar 자동 등록

Tempo 데이터소스는 `k8s/grafana/datasource-tempo.yaml` 의 ConfigMap (`grafana-datasource-tempo`) 으로 등록됩니다. 핵심 라인은 다음과 같습니다.

- **metadata.labels.grafana_datasource**: `"1"`. Grafana sidecar 가 watch 하는 라벨.
- **data.tempo-datasource.yaml**:
  - `type: tempo`, `access: proxy`, `url: http://tempo.monitoring:3200`, `uid: tempo`.
  - `jsonData.tracesToLogsV2`:
    - `datasourceUid: loki` (위 5장에서 등록된 Loki 데이터소스의 uid).
    - `filterByTraceID: true` (trace 의 traceID 로 Loki 로그를 필터).
    - `tags: [{ key: service.name, value: service_name }]` (OpenTelemetry resource attribute `service.name` 을 Loki label `service_name` 으로 매핑).

이 매핑 덕분에 Grafana 의 Tempo trace 화면에서 한 span 을 클릭하면 같은 `service_name` 라벨 + 동일 traceID 를 가진 Loki 로그로 바로 점프할 수 있습니다.
...
### 11.1 topologySpreadConstraints (같은 라벨 끼리)
...
### 11.2 cross podAntiAffinity (서로 다른 컴포넌트끼리)

`opentraum.io/heavy=true` 공통 라벨을 Prometheus, Loki, MariaDB, Tempo 가 모두 들고 있고, 각자의 affinity 에 `preferredDuringSchedulingIgnoredDuringExecution` `podAntiAffinity` 를 `weight=100`, `topologyKey=kubernetes.io/hostname` 으로 설정해 둡니다. 결과적으로 4개 무거운 Pod 가 서로 다른 노드를 선호하게 됩니다. `preferred` 라 노드 부족 시 강제하지 않아 Pending 위험이 없습니다.

### 11.3 두 장치를 함께 쓰는 이유

topologySpreadConstraints 는 같은 `labelSelector` 매칭 Pod 끼리만 분산을 정렬하므로 단일 replica StatefulSet (Prometheus, Loki, Tempo, MariaDB) 에서는 서로를 보지 못합니다. 반대로 cross podAntiAffinity 는 서로 다른 컴포넌트 사이에 작용하지만 같은 컴포넌트의 여러 Pod (예: kube-state-metrics) 가 한 노드에 몰리는 것은 막지 못합니다. 두 장치를 같이 쓰는 것은 이 두 축의 분산을 모두 챙기기 위함입니다.
...
- **Tempo monolithic + 10Gi PVC + 48h block_retention**: 단일 Pod 에서 distributor / ingester / querier 모두 실행, trace 는 48시간 보관.
- **Alloy 노드당 1 Pod (DaemonSet)**: 7개 노드 중 taint 가 없는 6개에서 기동되어 모든 일반 워크로드 노드의 Pod stdout 과 OTLP trace 를 수집합니다.
- **Grafana sidecar 자동 등록**: ConfigMap 라벨 `grafana_datasource: "1"` 만 붙이면 데이터소스가 자동으로 추가되어 수동 등록 작업이 0 입니다.
- **무거운 Pod cross anti-affinity preferred**: 노드 부족 시 강제 분산하지 않아 Pending 위험을 회피하면서, 여유 있을 때는 Prometheus / Loki / MariaDB / Tempo 가 서로 다른 노드로 자연스럽게 흩어집니다.

---

## 13. 트러블슈팅

### 13.1 무거운 Pod 한 노드 집중으로 메모리 압박
...
2. `kubectl get pod -n monitoring -o wide` 로 Prometheus, Loki, MariaDB, Tempo 가 같은 노드에 모여 있는지 확인합니다.
...
### 13.2 Tempo PVC AZ 종속으로 재스케줄 실패

증상: Tempo Pod 재기동 시 `VolumeBindingFailed` 또는 `FailedScheduling` 이벤트가 발생합니다.

진단 순서:

1. `kubectl describe pod -n monitoring tempo-0` 로 Events 를 확인합니다.
2. PVC `data-tempo-0` 가 바인딩된 AZ 와 사용 가능한 노드의 AZ 가 일치하는지 확인합니다.
...
### 13.4 Grafana datasource 미등록

증상: Grafana 에서 Tempo 또는 Loki 데이터소스가 보이지 않습니다.

진단 순서:

1. `kubectl get cm -n monitoring grafana-datasource-tempo --show-labels` 로 라벨 `grafana_datasource=1` 이 붙어 있는지 확인합니다.
2. `kubectl logs -n monitoring deploy/kube-prometheus-stack-grafana -c grafana-sc-datasources` 로 sidecar 가 ConfigMap 을 인식했는지 확인합니다.
3. 그래도 반영되지 않으면 `kubectl rollout restart deploy/kube-prometheus-stack-grafana -n monitoring` 으로 Grafana Pod 만 재기동합니다.

### 13.5 OTLP trace 가 도착하지 않음

증상: 애플리케이션은 정상이지만 Grafana Tempo 화면에 trace 가 보이지 않습니다.

진단 순서:

1. 애플리케이션 환경변수 `OTEL_EXPORTER_OTLP_ENDPOINT` 가 `http://alloy-otlp.monitoring:4318` (HTTP) 또는 `alloy-otlp.monitoring:4317` (gRPC) 로 설정되어 있는지 확인합니다.
2. `kubectl logs -n monitoring ds/alloy -c alloy --tail=200` 으로 Alloy receiver 와 exporter 로그를 확인합니다.
3. `kubectl exec -n monitoring tempo-0 -- wget -qO- http://localhost:3200/ready` 로 Tempo readiness 를 확인합니다.

### 13.6 Alertmanager 알림 미발송
