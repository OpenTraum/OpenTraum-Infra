...
Gateway는 외부 트래픽이 처음 닿는 진입점입니다. 이 테스트의 목표는 Gateway가 10,000 RPS를 받을 수 있도록 HPA, replica, node capacity, load-generator 배치, request/limit, probe 정책을 단계적으로 튜닝하고 각 변경의 효과를 근거로 남기는 것입니다.

중요한 전제는 "부하를 받는 서비스 용량"과 "부하를 만드는 generator 용량"을 분리하는 것입니다. 서비스 노드와 HPA를 튜닝하는 것은 10,000 RPS 달성 목표에 포함됩니다. 다만 한 실험 안에서 여러 값을 동시에 바꾸면 어떤 변경이 효과를 냈는지 알 수 없으므로, 매 phase마다 target capacity budget을 고정하고 변경점을 명시합니다.

부하 발생기는 별도 namespace와 별도 node pool에서 늘립니다. 이 분리는 local PC 병목을 제거하고, 부하 발생 Pod가 Gateway Pod와 같은 node 자원을 경쟁하지 않게 하기 위한 조건입니다. 이후 Gateway/HPA/node 튜닝은 별도 phase로 기록합니다.
...
- route miss로 404가 나더라도 Gateway CPU/HPA 반응 관측 가능

이에 따라 `GET /api/__loadtest__` 경로를 사용합니다. `Non-2xx or 3xx responses`는 이 경로가 downstream route miss로 404를 반환하기 때문에 발생하며, 이 문서에서는 실패율이 아니라 Gateway 통과 트래픽으로 해석합니다.
...
| target tuning | Gateway HPA, replica, 일반 서비스 node, resource, probe를 단계적으로 변경 | 별도 namespace/node pool의 wrk2 Pod 확장 | 10,000 RPS 달성을 위한 튜닝 효과 검증 |

따라서 Gateway/HPA/node를 늘린 결과를 버리지 않습니다. 대신 "기준 구성 결과"와 "튜닝 구성 결과"를 분리해 기록합니다.
...
이 runner의 목적은 local PC 한 대의 CPU/네트워크 한계를 제거하는 것입니다. Gateway가 뜨는 일반 서비스 노드 증설과 HPA 상향은 별도 target tuning phase로 분리해 기록합니다.
...
결론적으로 기존 baseline만으로는 10,000 RPS를 지원한다고 말할 수 없습니다. 수현님 피드백에 따라 부하 발생기를 local PC가 아니라 EKS 내부 wrk 전용 노드에 올리고, wrk2 Pod 17개를 한 wrk 노드에 모아 Gateway와 분리했습니다. 이 조건에서 60초 smoke는 10,000 RPS에 매우 근접했지만, 300초 지속 실험은 wrk 노드 CPU와 timeout이 동시에 터지며 실패했습니다. 다음 단계는 서비스 노드를 무한정 늘리는 것이 아니라, 실서비스 재현성을 유지한 채 단일 wrk 노드의 부하 발생 한계와 Gateway/Gateway node 병목을 분리해 해결하는 것입니다.
...
`wrk2` Pod를 `opentraum-loadtest` namespace에 띄우고, Gateway와 다른 node pool로 분리해 local PC 병목을 제거했습니다. 이 방식은 수현님 피드백처럼 "부하 발생기를 여러 서버처럼 늘리는" 목적에 가깝습니다.
...
|---|---:|---:|---:|---:|---|---:|---|---|---|
| one wrk node baseline | 10,000 | 17 | 128 | 300s | HPA baseline에서 시작 | 3,432.76 | read 5,092 / write 4,947 / timeout 255 | Gateway ready 하락 후 7 replicas 회복 | cold-start와 Gateway 안정성 영향 큼 |
...
| prewarm10 c256 | 10,500 | 17 | 256 | 60s | 10 replicas | 3,248.41 | read 253 / write 1,567 / timeout 53,901 | Gateway 10/10이나 timeout 급증 | target 상향 실패 |
| prewarm10 c256 | 10,150 | 17 | 256 | 60s | 10 replicas | 6,145.33 | read 863 / write 251 / timeout 0 | 일부 Gateway restart 관측 | 10k 초과 안정화 실패 |
| prewarm10 c256 | 10,000 | 17 | 256 | 300s | 10 replicas | 2,576.56 | read 1,359 / write 7,171 / timeout 385,251 | Gateway 10/10 유지, wrk 노드 CPU 100%대 | 300초 지속 실패 |

60초 c256 smoke는 10,000 RPS에 112.59 RPS 부족한 수준까지 접근했고, Gateway ready도 10/10을 유지했습니다. 그러나 같은 조건을 300초로 늘리면 부하 발생기 쪽 wrk 노드 CPU가 100%대까지 올라가고 timeout이 대량 발생했습니다. 따라서 현재 결과는 "Gateway가 10,000 RPS를 처리했다"가 아니라 "단일 wrk 전용 노드 조건에서 60초 단기 주입은 거의 가능하지만, 300초 지속 주입은 부하 발생기와 Gateway/node 병목을 더 분리해야 한다"로 해석합니다.

참고로, 일반 서비스 노드와 Gateway replica를 더 크게 열고 wrk2 Pod 수도 늘린 broad tuning smoke에서는 9,978 RPS까지 관측했습니다. 다만 이 조건은 이번 기준인 "wrk용 노드만 1개 추가"와 다르므로 최종 capacity 판정에는 사용하지 않고, 10,000 RPS 달성 가능성을 본 참고 신호로만 둡니다.
...
부하 발생기를 EKS 내부로 옮긴 뒤에는 해석이 더 명확해졌습니다. wrk 전용 노드 1개 + wrk2 Pod 17개 + Gateway 10 replica pre-warm + connection 256 조건에서 60초 9,887 RPS까지 접근했습니다. 그러나 10,500 RPS, 10,150 RPS 재시도는 timeout 또는 Gateway restart로 무너졌고, 10,000 RPS 300초 지속 실험도 timeout 대량 발생으로 실패했습니다.
...
| 10000 RPS 목표 | wrk 전용 노드 1개 + wrk2 17 Pod + c256 조건을 기준으로 병목 제거 | 60초 9887 RPS 근접, 300초 지속 실패 |
...
| 우선순위 | 제안 | 이유 |
|---:|---|---|
| 1 | wrk 전용 노드의 instance type 또는 CPU capacity 상향 | 300초 10,000 RPS에서 wrk 노드 CPU가 100%대까지 올라가 timeout 대량 발생 |
...
- Gateway가 올라가는 일반 서비스 노드는 실험 범위 안에서 고정하고, 변경이 필요하면 별도 tuning phase로 분리
...
| 일반 서비스 node count/type | 실서비스 재현성 유지와 target capacity budget 고정 |
...
다음 실험의 1순위는 9887 RPS 근접 조건을 재현한 뒤, 같은 조건을 300초 동안 유지하도록 만드는 것입니다.

| 순서 | 튜닝 항목 | 기대 효과 |
|---:|---|---|
| 1 | wrk 전용 노드 1개에서 c256 17 Pod 60초 9887 RPS 재현 | 실험 재현성 확보 |
