...
부하 발생기는 별도 namespace와 wrk 전용 노드에서 실행합니다. 이 분리는 workstation 병목을 제거하고, 부하 발생 Pod가 Gateway Pod와 같은 node 자원을 경쟁하지 않게 하기 위한 조건입니다. 이후 Gateway/HPA 튜닝은 서비스 노드 7개 고정 조건을 유지한 별도 phase로 기록합니다.
...
|---|---|---|---|
| baseline | 기존 Gateway HPA와 기존 서비스 노드 범위 유지 | EKS `opentraum-loadtest` namespace의 wrk2 Job | 현재 출발점 확인 |
...
실행 스크립트는 [../scripts/load-test/gateway-hpa-k8s-wrk2-runner.sh](../scripts/load-test/gateway-hpa-k8s-wrk2-runner.sh)입니다. 호환용 entrypoint [../scripts/load-test/gateway-hpa-load-test.sh](../scripts/load-test/gateway-hpa-load-test.sh)도 같은 Kubernetes Job runner로 위임하며, legacy `MODE=wrk*`, `WRK_BIN`, `WRK2_BIN` 같은 workstation generator 옵션은 실패 처리합니다.
...
이 runner의 목적은 developer workstation의 CPU/네트워크 한계를 실험 변수에서 제거하는 것입니다. Gateway가 뜨는 서비스 노드는 7개로 고정하고, HPA/resource/probe 조정은 별도 target tuning phase로 분리해 기록합니다.
...
| 구간 | 핵심 결과 | 판정 |
|---|---|---|
| 500 RPS | 과거 workstation 결과는 참고 이력으로만 보관. 신규 판정은 EKS wrk2 Job으로 재검증 | EKS 기준 재검증 필요 |
| 900~1000 RPS | 과거 workstation 결과에서는 HPA scale-out 관측. 신규 판정은 EKS wrk2 Job만 사용 | 최소 2 replicas 후보, EKS 기준 재검증 필요 |
| 1500 RPS | 과거 workstation 결과는 capacity 판정에서 제외. EKS wrk2 Job matrix로 재검증 | EKS 기준 재검증 필요 |
| 2000 RPS | EKS `wrk2` 17 Pod는 2005 RPS 달성 | RPS는 통과, p99 1초대라 SLO는 별도 판단 필요 |
| 5000 RPS | baseline 범위에서는 안정 통과하지 못함. cold-start/probe 재시작과 timeout 확인 | 튜닝 필요 |
| 10000 RPS | wrk 전용 노드 1개 + wrk2 17 Pod 조건에서 60초 9887 RPS까지 근접한 기록은 있으나, 후속 재현에서는 6 replicas/c128의 7442 RPS가 최신 안정값. 8~10 replicas/c256은 Gateway restart와 socket error 발생 | 10,000 RPS 안정 달성은 아직 실패. Gateway probe/resource를 GitOps manifest로 튜닝한 뒤 재검증 필요 |

결론적으로 기존 baseline만으로는 10,000 RPS를 지원한다고 말할 수 없습니다. 리뷰 피드백에 따라 부하 발생기를 EKS 내부 wrk 전용 노드에 올리고, wrk2 Pod 17개를 한 wrk 노드에 모아 Gateway와 분리했습니다. 이 조건에서 60초 smoke는 10,000 RPS에 근접한 기록이 있었지만, 후속 재현에서는 Gateway liveness restart가 다시 발생했고 300초 지속 실험도 실패했습니다. 다음 단계는 서비스 노드를 늘리는 것이 아니라, 전체 11개/서비스 7개 노드 조건을 유지한 채 Gateway probe/resource 튜닝을 GitOps manifest 기준으로 반영해 병목을 다시 분리하는 것입니다.
...
`wrk2` Pod를 `opentraum-loadtest` namespace에 띄우고, Gateway와 다른 node pool로 분리해 developer workstation 병목을 제거했습니다. 이 방식은 부하 발생기를 애플리케이션 서비스 노드와 분리해 client 병목을 줄이는 목적입니다.
...
### 6.3 wrk 전용 노드 1개 + wrk2 17 Pod 결과
...
후속 재현에서는 Gateway repo code를 수정하지 않고, ConfigMap에 임시 Gateway-only 204 route를 추가해 downstream API를 제외했습니다. 실험 후 route는 제거했고 Gateway는 1 replica 기준으로 재시작했습니다.

| Phase | Target RPS | Pods | Connections | Duration | Gateway pre-warm | Actual RPS | Socket errors | Gateway 관측 | 판정 |
|---|---:|---:|---:|---:|---|---:|---|---|---|
| auth204 r4 c128 | 10,000 | 17 | 128 | 60s | 4 replicas | 4,841.99 | timeout 23 | restart 0 | 부족 |
| auth204 r6 c128 | 10,000 | 17 | 128 | 60s | 6 replicas | 7,441.91 | 없음 | restart 0 | 최신 재현 중 최선 |
| auth204 r8 c128 | 10,000 | 17 | 128 | 60s | 8 replicas | 6,246.62 | timeout 76 | restart 0 | 처리량 하락 |
| auth204 r8 c256 | 10,000 | 17 | 256 | 60s | 8 replicas | 2,383.11 | read/write/timeout 다수 | liveness restart | 중단 기준 |
| auth204 r10 c256 | 10,000 | 17 | 256 | 60s | 10 replicas | 4,472.83 | read/write/timeout 다수 | liveness restart | 중단 기준 |

이 재현에서는 단순히 Gateway replicas를 8~10으로 올리거나 wrk2 connection을 256으로 늘리는 방식이 안정적인 개선으로 이어지지 않았습니다. 특히 c256 구간에서는 health probe timeout 이후 liveness restart가 발생했습니다. live Deployment patch로 probe를 완화하려 했지만 ArgoCD가 Gateway Deployment를 GitOps 기준으로 되돌렸기 때문에, 다음 검증은 임시 `kubectl patch`가 아니라 Gateway manifest 변경 PR 단위로 진행해야 합니다.
...
10,000 RPS는 현재 baseline Gateway 구성의 capacity로는 선언할 수 없습니다. 제외된 workstation generator 실험은 client 병목과 외부 경로 영향이 섞였으므로 capacity 판정에서 배제합니다. EKS 내부 wrk2 실험은 2000 RPS까지는 유효했지만 5000 RPS부터 안정성이 무너졌습니다.
...
| 1500 RPS 목표 | EKS wrk2 Job으로 재검증 전까지 보수적으로 판단 | workstation generator 결과는 판정에서 제외 |
...
| 10000 RPS 목표 | Gateway probe/resource 튜닝을 GitOps manifest로 반영한 뒤 wrk 전용 노드 1개 + wrk2 17 Pod 조건 재검증 | 60초 9,887 RPS 근접 기록은 있으나, 후속 재현에서 liveness restart와 socket error 발생 |
...
4. 10,000 RPS 달성이 목표이므로 서비스 노드는 고정 범위로 두고, Gateway liveness/readiness probe와 CPU limit을 먼저 tuning matrix로 관리
5. 60초 smoke에서 socket error와 Gateway restart가 사라진 뒤에만 300초 지속 실험으로 승격
...
| 4 | Gateway pre-warm은 6 replicas 안정 기준에서 시작하고, probe/resource 튜닝 후 8/10으로 재비교 | 후속 재현에서 6 replicas/c128이 최신 안정값이고 8/10 c256은 restart 발생 |
| 5 | Gateway CPU limit과 liveness/readiness probe timeout 비교 실험 | 10k 주입 시 health probe timeout으로 liveness restart가 발생했기 때문 |
...
| 순서 | 튜닝 항목 | 기대 효과 |
|---:|---|---|
| 1 | EKS wrk2 Job c256 17 Pod 60초 9,887 RPS 재현 | 실험 재현성 확보 |
