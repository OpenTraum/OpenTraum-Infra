...
- [6. 자동 스케일링 (현재 상태)](#6-자동-스케일링-현재-상태)
...
라이브 클러스터에는 OpenTraum 도메인 전용 PriorityClass 4종이 등록되어 있습니다. 매니페스트 출처는 [../manual/k8s/priorityclass.yml] 한 파일이며, 모두 `globalDefault: false`로 설정되어 라벨이 없는 Pod는 클러스터 기본값(0)을 사용합니다.
...
| **opentraum-best-effort** | **-100** | PreemptLowerPriority | monitoring-service, flux-image, nemotron-vllm (opentraum ns), Prometheus·Grafana·Loki·Alertmanager·Alloy·kube-state-metrics·operator·loki-gateway (monitoring ns), dcgm-exporter (gpu-monitoring ns), argocd 6종 |

**4단계 분류의 의도.** 클러스터 자원이 부족할 때 어떤 Pod 가 먼저 양보해야 하는가를 명시적으로 정합니다. 가장 위 등급(opentraum-high)에는 좌석 예매 흐름의 SAGA 핵심 경로(gateway/payment/reservation)와 분산 락을 제공하는 redis 가 묶이고, 그 다음(opentraum-medium)에 일반 도메인 서비스(auth/user/event), 그 다음(opentraum-low)에 정적 자산 서빙(web), 가장 아래(opentraum-best-effort, value=-100)에 "장애 발생 시 먼저 evict 되어도 되는" 워크로드가 묶입니다. opentraum-best-effort 그룹은 운영 가시성(모니터링 인프라), 학습용 GPU 시뮬레이터(flux-image/nemotron-vllm), GitOps 컨트롤 플레인(argocd), 클러스터 내부 모니터링 백엔드(monitoring-service) 로 구성되며 일시적 종료가 사용자 트래픽에 직접 영향을 주지 않습니다.

**preemptionPolicy=PreemptLowerPriority 의 의미.** 노드의 CPU 또는 메모리가 부족해 새 Pod 를 스케줄할 자리가 없을 때, kube-scheduler 는 자기보다 낮은 priority value 를 가진 Pod 를 강제 종료(preempt)하고 그 자리를 차지할 수 있습니다. value 1000 인 gateway Pod 가 새로 스케줄되어야 하는데 노드에 여유가 없다면, kube-scheduler 가 가장 낮은 -100 인 best-effort 그룹부터 차례로 evict 시도합니다. 반대로 같은 등급(1000 대 1000) 또는 더 높은 등급의 Pod 는 절대 빼앗기지 않으므로 등급 자체가 보호 경계 역할을 합니다.

**default(0) 와 best-effort(-100) 의 분리 의미.** PriorityClass 가 명시적으로 부여되지 않은 Pod 는 기본값 0 을 받습니다(redis, mariadb, kafka, ingress-nginx, cert-manager 등 인프라 워크로드 다수가 이 등급). best-effort 그룹은 -100 으로 명시적으로 더 낮게 두어, 자리 부족 시 default(0) 그룹보다 먼저 양보하도록 강제합니다. 이는 "기본값 0 보다도 낮춰서 운영 가시성/학습 시뮬레이터를 가장 먼저 희생시키되 인프라 워크로드(데이터 계층/네트워크 계층/인증서)는 명시적 PC 없이도 자동으로 보호받도록" 한 설계입니다.

**globalDefault=false 가 만드는 안전장치.** 만약 어느 PriorityClass 가 globalDefault=true 였다면 라벨이 없는 모든 Pod 가 그 등급을 자동으로 받게 되어, 신규 워크로드가 의도치 않게 high 를 차지하거나 best-effort 로 떨어지는 사고가 가능합니다. 4종 모두 false 로 두어 명시적 지정만 받도록 했습니다.
...
**왜 gateway는 maxUnavailable=0 인가.** gateway는 외부 트래픽이 처음 닿는 단일 진입점이라 종료되는 Pod 가 그대로 다운으로 이어지면 트래픽 손실이 즉시 발생합니다. PDB=0은 "voluntary disruption(노드 drain, 이미지 업데이트, kubectl delete pod 등 자발적 중단)으로 인한 가용 Pod 감소를 절대 허용하지 않는다"는 약속입니다. gateway Deployment 의 라이브 `replicas=1 + RollingUpdate(maxSurge=1, maxUnavailable=0)` 조합과 결합되면, 업데이트 시 새 Pod 1개를 먼저 띄워 Ready 상태가 된 뒤(maxSurge=1) 이전 Pod 가 종료되어 가용 Pod 수가 1 -> 2 -> 1 로 흐릅니다. PDB=0 은 이 약속을 클러스터 차원에서 강제하는 자물쇠 역할이지만, 단일 replica 환경에서는 노드 drain 을 영구 block 하므로 노드 그룹 롤링 작업 시점에 한해 maxUnavailable=1 로 완화하는 운영 변경을 별도로 검토해야 합니다.
...
**maxSkew=1 + ScheduleAnyway.** 동일 라벨 셋의 Pod 수가 노드별로 1 이상 차이 나지 않도록 유도하되, 자리가 부족하면 같은 노드 배치도 허용합니다. gateway 는 라이브 `replicas=1` 이라 단일 Pod 라 분산 효과가 나타나지 않으나, 다른 multi-replica 워크로드(예: web 의 과거 replicas=2 시점)에서 동일 라벨 Pod 가 서로 다른 노드로 분산되었음을 라이브 관측으로 확인했습니다.
...
현재 클러스터에 자동 스케일러는 설치되어 있지 않습니다. 이전에는 KEDA 2.16.1 (Kubernetes Event-Driven Autoscaler) 가 `keda` 네임스페이스에 Helm release `keda` 로 설치되어 있었으나, ScaledObject 가 정의된 적 없어 실질적으로 사용되지 않은 상태로 남아 있었습니다. 사용하지 않는 컨트롤 플레인이 자원과 운영 부담만 차지하는 상태였기에 본 산정 시점에 helm uninstall 로 제거하고 ns 도 삭제했습니다.

향후 트래픽 부하 테스트 (08 TRAFFIC-TEST 참고) 시점에 RPS 기반 또는 Kafka lag 기반 스케일이 필요해지면 KEDA 또는 표준 HPA 를 다시 도입합니다. 표준 HPA 는 CPU/Memory metric 만 트리거 가능하고, KEDA 는 Prometheus query (RPS), Kafka consumer lag, Redis stream 등 외부 이벤트 소스를 트리거로 받습니다. OpenTraum 의 트래픽 패턴 (gateway 진입점 + reservation-service 의 Kafka 컨슈머) 에는 KEDA 의 Kafka lag scaler 가 적합한 후보입니다.
...
**RollingUpdate maxSurge=1, maxUnavailable=0.** "먼저 새 Pod를 추가한 뒤 기존 Pod를 내린다"는 정책입니다. 라이브 `replicas=1` 인 gateway 기준으로 업데이트 도중 Ready Pod 수가 1 -> 2 -> 1 로 변하므로 배포 중 트래픽 차단 시간 창이 없습니다. 3장의 PDB=0 과 짝을 이루어 정상 배포 흐름의 무중단 롤아웃을 보장하며, Pod 강제 삭제(카오스) 시에는 새 Pod 의 기동 시간(48.7s) 동안 일시적 자가 복구 지연 가능성이 남아있습니다 (03 §8 트러블슈팅 참고).
...
sequenceDiagram
    ...
    API->>CTRL: ReplicaSet 관찰: 0/1 Ready
...
**해석.** 실험 결과는 본 장의 정책 묶음(replicas=1 + RollingUpdate maxSurge=1 maxUnavailable=0 + Probe 65s + PDB=0)이 단일 Pod 강제 삭제라는 충격을 흡수해 40초 안에 정상 상태로 복귀시킨다는 사실을 입증합니다. deviated=false는 Chaos Toolkit이 가설 위반을 감지하지 못했다는 뜻이며, 단일 replica 라 새 Pod 의 기동 시간(48.7s) 동안 일시적 자가 복구 지연 가능성은 남아있습니다 (03 §8 트러블슈팅 참고).
...
컴퓨트 비용은 노드 한 대당 vCPU/Memory 단가로 곱해지므로, 워크로드별 requests/limits 정책이 곧 노드 수와 직결됩니다. OpenTraum은 8개 서비스 모두 SAGA 핵심 경로 / 핵심 의존성 / 정적 자산 서빙 (web) 이라 전부 `request = limit` 으로 묶어 Guaranteed QoS 로 통일하고, 다른 ns 의 Burstable Pod (Prometheus·Grafana 등) 가 먼저 evict 되어 메모리가 회수되는 구조로 운영합니다. CPU 는 서비스별 startupProbe 한도와 기동 실측을 기반으로 200~350m 차등, Memory 는 idle + 30% 헤드룸 + 64Mi 단위 반올림 후 백엔드 7 종을 512Mi 로 통일했습니다. JVM 옵션도 512Mi 한도에 정합하도록 조정되어 OOMKilled 빈도와 GC 폭주를 줄이는 방향으로 자원이 배분됩니다.

| 정책/설정 | 적용 위치 | 효과 | 근거(왜 이 값으로) | 출처 |
|---|---|---|---|---|
| gateway requests=limits cpu=300m mem=512Mi (Guaranteed) | gateway Deployment | request=limit 으로 evict 보호, 튜닝 전 cpu req/lim 250m/500m, mem req/lim 128Mi/256Mi 에서 현재 값으로 갱신 | Spring Cloud Gateway 라우팅 + JWT 검증, startup 시 JIT 워밍업 마감 시간 65초 한도 안에 들어오는 cpu 300m 채택 (실측 48.7s, 03 §4). mem 512Mi 는 idle + 30% 헤드룸 안. | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md) |
| auth requests=limits cpu=300m mem=512Mi (Guaranteed) | auth-service | request=limit Guaranteed, cpu 300m 으로 startup 46.9s, 65초 한도 마진 +38% | auth 의 의존성(JPA + Redis) 무게 < user 의존성(R2DBC + schema 적용 + Spring Data Redis) 무게라 user 가 더 큰 cpu 필요. 200m 환경 startup 실측 auth 66.7s / user 81.4s, 차등 채택 (03 §4). 250m 채택 후 Burstable→Guaranteed 전환으로 burst 가 차단되며 startup 직후 Redis health check 가 65s 한도 안에 끝나지 못해 CrashLoop 가 발생, 300m 으로 재산정 (03 §8.6). | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md) |
| user requests=limits cpu=350m mem=512Mi (Guaranteed) | user-service | request=limit Guaranteed, cpu 350m 으로 startup 65초 한도 통과 | auth 의 의존성(JPA + Redis) 무게 < user 의존성(R2DBC + schema 적용 + Spring Data Redis) 무게라 user 가 더 큰 cpu 필요. 200m 환경 startup 실측 auth 66.7s / user 81.4s, 차등 채택 (03 §4). | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md) |
| event requests=limits cpu=200m mem=512Mi (Guaranteed) | event-service | request=limit Guaranteed, cpu 200m 통일 | event-service 는 startupProbe 미정의라 65초 한도 미적용. 200m 환경 기동 시간 76.7s 가 길지만 죽지 않음. mem 512Mi 는 다른 백엔드와 통일, idle 291Mi + 30% 헤드룸 안 (03 §4). | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md) |
| reservation/payment requests=limits cpu=200m mem=512Mi (Guaranteed) | reservation-service, payment-service | Guaranteed 등급으로 evict 우선순위 최저, 트랜잭션 핵심 경로 보호 | 두 서비스 모두 startupProbe 미정의. 200m 환경 기동 시간 93~99s 길지만 죽지 않음. mem 512Mi (03 §4). 좌석/결제 SAGA 핵심 경로라 Guaranteed 유지. | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md) |
| monitoring requests=limits cpu=200m mem=512Mi (Guaranteed) | monitoring-service | request=limit Guaranteed, 백엔드 7 종 중 하나로 동일 정책 적용 | Spring Boot 모니터링 서비스. startupProbe 한도 95초 (failureThreshold=30) 라 200m 환경 기동 65.2s 도 충분히 통과 (03 §4). | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md) |
| web requests=limits cpu=50m mem=64Mi (Guaranteed) | web Deployment | 정적 자산 서빙 nginx로 가장 가벼움, replica 2→1 축소 | 정적 자산 서빙 nginx, 측정 idle 1m/3Mi. 50m/64Mi = idle 50/20배 여유. replica 2→1 축소 (03 §3.5/§4). | [03 §3.5](OPENTRAUM-INFRA-03-WORKLOAD.md) |
...
| node over-commit (limits 합 추이) | 클러스터 전역 | 튜닝 전 limits 합 202% 였으나 03 의 자원 재산정으로 78.5% 로 감소. 자세한 변화는 [03 §7](OPENTRAUM-INFRA-03-WORKLOAD.md#7-정량--튜닝) 참고 | request=limit 통일 후 requests 합 = limits 합 이라 over-commit 자체가 사라짐. 동시 burst 가정이 필요 없는 보수적 구조로 전환. | [01 CLUSTER](OPENTRAUM-INFRA-01-CLUSTER.md) |
...
**`MaxRAMPercentage=75.0`이 만든 OOMKilled 안전선.** limit이 512Mi인 컨테이너에서 JVM이 디폴트 동작으로 힙을 잡으면 cgroup 한도와 무관하게 호스트 RAM 기준으로 산정되어 limit을 초과합니다. 컨테이너 환경에서 limit의 100%를 힙으로 잡으면 메타스페이스/스레드/네이티브 영역이 들어갈 자리가 없어 JVM 자체가 시동조차 못 하는 경우도 발생합니다. JVM 공식 권장 범위가 65~75%인 이유는 비-힙 오버헤드가 보통 25~35% 사이에 분포하기 때문이고, 그 중 상한값인 75%를 채택해 힙을 384Mi 까지 잡으면서 비-힙 128Mi 를 안정적으로 확보했습니다. 75% 고정으로 힙 상한을 384Mi로 묶고 나머지 128Mi를 비-힙 영역에 양보함으로써, GC 압박 시에도 OOMKilled가 컨테이너 내부 사고로만 국한되고 노드 전체 메모리 압박으로 번지지 않습니다. 이 정책이 없었다면 동일 노드의 다른 Pod까지 evict가 전염되어 결과적으로 노드 추가 비용을 불러왔을 가능성이 큽니다.

**8개 서비스 모두 Guaranteed 통일이 만드는 evict 순서.** kubelet은 노드 메모리 압박 시 Guaranteed > Burstable > BestEffort 순으로 보호합니다. OpenTraum 은 8개 서비스 모두 SAGA 핵심 경로이거나 핵심 의존성이라 모두 evict 보호 우선이라는 결정으로, 백엔드 7 종 + web 모두 `requests = limits` 로 묶어 Guaranteed 로 통일했습니다. 같은 노드에 떠 있는 다른 ns 의 Burstable Pod (Prometheus, Grafana, Loki 등) 가 먼저 evict 되어 메모리가 회수되는 구조라(03 §1.5.3), 트래픽 폭증 중 분산 트랜잭션 일관성이 깨질 위험이 사라졌습니다. 트레이드오프는 burst 헤드룸이 0 이라는 점이지만, idle 사용량이 채택값의 1~5% 에 머무르는 백엔드 특성상 burst 가 거의 의미 없다는 03 §1.5.5 의 분석을 그대로 따릅니다. 과거 "reservation/payment 만 Guaranteed, 나머지 Burstable" 구조 대비 노드 비용은 거의 변함이 없는데(서비스별 차등 산정으로 limits 합이 오히려 감소, 03 §7), evict 보호 범위는 8개 전체로 확장됐습니다.
...
| GITHUB_TOKEN 채택 | workflow auth | PAT 발급/관리 0건, 자동 발급/폐기, 무한루프 자동 차단. 튜닝 전() PAT(GIT_TOKEN) 수동 발급/관리 + actor 체크 우회 패턴 -> 튜닝 후 자동 발급/폐기 + 플랫폼 레벨 무한루프 차단으로 전환 | PAT 발급/회전/관리 0건. 자동 발급/폐기로 토큰 노출 위험 감소. push 시 워크플로 재트리거를 GitHub 가 플랫폼 차원 자동 차단해 무한루프 방지(07 §11 무한루프 방지 정책). | [07 CICD](OPENTRAUM-INFRA-07-CICD.md) |
...
| 정책/설정 | 적용 위치 | 효과 | 근거(왜 이 값으로) | 출처 |
|---|---|---|---|---|
| gateway replicas=1 + PDB maxUnavailable=0 + RollingUpdate maxSurge=1 maxUnavailable=0 | gateway Deployment + PDB | 단일 replica 환경에서 RollingUpdate 시 새 Pod 가 Ready 된 뒤 옛 Pod 종료, 배포 중 트래픽 차단 0. 단일 replica 라 Pod 강제 삭제 시 새 Pod 기동 시간(48.7s) 동안 일시적 자가 복구 지연 가능 | 외부 트래픽의 단일 진입점. 라이브 replicas=1 (Spring 7개 서비스와 일관성, 트래픽 < 단일 Pod 처리 한도). RollingUpdate maxSurge=1/maxUnavailable=0 으로 새 Pod 가 Ready 된 뒤 옛 Pod 종료 (Pod 수 1 -> 2 -> 1). 단, PDB maxUnavailable=0 은 단일 replica 환경에서 drain 을 영구 block 하므로 노드 작업 시 maxUnavailable=1 로 완화 검토 필요. | [03 WORKLOAD](OPENTRAUM-INFRA-03-WORKLOAD.md) · 본 장 §3 |
...
| Probe startup ≤ 65s (gateway/auth/user) 또는 ≤ 95s (monitoring) + readiness 10s + liveness 30s | startupProbe 정의 서비스 (gateway/auth/user/monitoring) 한정 | 부팅 워밍업과 장애 감지 균형, 조급한 재시작 0. event/payment/reservation 은 startupProbe 미정의로 본 한도 미적용 | Spring Boot 부팅 평균 30~50s. startup 한도는 gateway/auth/user 65s, monitoring 만 95s (failureThreshold=30). readiness 10s 는 빠른 트래픽 허용 결정, liveness 30s 는 짧은 GC 정지에 재시작 안 하도록 여유. | 본 장 §4 · [03 §6.2](OPENTRAUM-INFRA-03-WORKLOAD.md#62-probe--종료-정책-한-장-표) |
...
**gateway 무중단 배포 조합의 의미.** gateway replicas 는 1 (라이브 확인). 무중단 배포는 RollingUpdate maxSurge=1 / maxUnavailable=0 으로 새 Pod 가 Ready 된 뒤 옛 Pod 가 종료되는 방식이며, 배포 진행 중에는 Pod 수가 1 -> 2 -> 1 로 흐르므로 트래픽 차단 시간 창이 없습니다. 단일 replica 라는 점은 Pod 강제 삭제(카오스) 시 새 Pod 의 기동 시간(48.7s) 동안 일시적 자가 복구 지연이 발생할 수 있다는 의미이며, 03 §8 트러블슈팅에서 상세히 다룹니다. 종료 측에서는 terminationGracePeriod=40s + Spring graceful 30s 조합이 종료되는 Pod 의 in-flight 요청을 SIGKILL 전에 끝내야 사용자 단에 5xx 가 노출되지 않습니다. 즉, 본 장 §8의 42.51초 복구 수치는 §3(PDB), §4(Probe + 종료 정책), §5(분산 정책) 정책의 조합이 만든 결과이지 단일 설정의 효과가 아닙니다. 또한 PDB maxUnavailable=0 은 단일 replica 환경에서 drain 을 영구 block 하므로, 노드 그룹 롤링 작업 시점에는 maxUnavailable=1 로 완화하는 운영 변경을 별도로 검토해야 합니다.
...
5. **HPA 미사용 환경에서 트래픽 폭증.** 현재 자동 스케일러가 설치되어 있지 않으므로 자동 스케일이 동작하지 않습니다. 폭증 감지 시 수동 스케일은 `kubectl scale deploy/{name} -n opentraum --replicas=N`이며, 종료 후에는 매니페스트와 다른 상태가 되므로 ArgoCD sync에서 되돌려질 수 있다는 점을 인지합니다. 항구적 대응은 KEDA ScaledObject 도입(예: reservation-service의 Kafka lag 트리거)입니다.
