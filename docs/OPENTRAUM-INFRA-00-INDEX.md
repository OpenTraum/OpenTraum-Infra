...
| [08 GATEWAY HPA LOAD TEST](OPENTRAUM-INFRA-08-GATEWAY-HPA-LOAD-TEST.md) | Gateway HPA 부하 테스트 | EKS wrk2 Job 기준 Gateway HPA 부하 테스트와 10,000 RPS 달성을 위한 HPA/node/loadgen 튜닝 방향을 정리합니다. |
...
| Gateway HPA scale-out 기준 | 미측정 | 500 RPS=1 replica, 900~1000 RPS부터 scale-out, 1500 RPS=4 replicas, EKS wrk2 17 Pod로 2000 RPS 달성. wrk 전용 노드 1개 + 17 Pod 조건에서 10,000 RPS 60초 smoke는 9887 RPS까지 근접했지만, 후속 재현에서는 6 replicas/c128의 7442 RPS가 최신 안정값이고 300초 지속은 실패 | [08 §5~§8](OPENTRAUM-INFRA-08-GATEWAY-HPA-LOAD-TEST.md) |
