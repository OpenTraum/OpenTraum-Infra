...
| [01 CLUSTER](OPENTRAUM-INFRA-01-CLUSTER.md) | EKS 클러스터 / 노드 / 스토리지 | ap-northeast-2 의 9개 노드(t3.medium 7 + g5.xlarge 2)와 50Gi 영구 스토리지를 정리합니다. |
...
| [03 WORKLOAD](OPENTRAUM-INFRA-03-WORKLOAD.md) | 네임스페이스 / 앱 워크로드 카탈로그 | opentraum 네임스페이스의 백엔드 7 종 + web 1 종 (총 8개 서비스) 을 매니페스트 라인 단위로 분석합니다. |
...
| [06 OPERATIONS](OPENTRAUM-INFRA-06-OPERATIONS.md) | 운영 정책 / 안정성 / 카오스 | PriorityClass, PDB, Probe, Affinity, 자동 스케일링 정책, Chaos Toolkit 검증 결과까지 다룹니다. |
| [07 CICD](OPENTRAUM-INFRA-07-CICD.md) | CI/CD 파이프라인 | GitHub repo 에 push 한 한 번의 커밋이 EKS 의 Pod 가 되기까지의 모든 단계를 정리합니다. |
| [08 TRAFFIC TEST](OPENTRAUM-INFRA-08-TRAFFIC-TEST.md) | Gateway 트래픽 테스트 | EKS wrk2 Job 기준 Gateway 트래픽 테스트와 10,000 RPS 달성을 위한 HPA/node/loadgen 튜닝 방향을 정리합니다. |
...
| 노드 수 | 9 (t3.medium 7 + g5.xlarge 2) |
| 총 vCPU / Memory | 22 vCPU / 약 60Gi |
...
| 항목 | 튜닝 전 | 튜닝 후 | 출처 |
|---|---|---|---|
| gateway replicas | 1 | 1 유지 — Spring 7개 서비스와 일관성, 트래픽 < 단일 Pod 처리 한도 | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md), [06 §9.5](OPENTRAUM-INFRA-06-OPERATIONS.md) |
| Pod 강제 삭제 후 자가 복구 | 60초 이상 (카오스 deviated) | 42.51초 (카오스 completed) | [06 §8.2](OPENTRAUM-INFRA-06-OPERATIONS.md) |
| Gateway HPA scale-out 기준 | 미측정 | 500 RPS=1 replica, 900~1000 RPS부터 scale-out, 1500 RPS=4 replicas, EKS wrk2 17 Pod로 2000 RPS 달성. wrk 전용 노드 1개 + 17 Pod 조건에서 10,000 RPS 60초 smoke는 9887 RPS까지 근접했지만, 후속 재현에서는 6 replicas/c128의 7442 RPS가 최신 안정값이고 300초 지속은 실패 | [08 §5~§8](OPENTRAUM-INFRA-08-TRAFFIC-TEST.md) |
| Probe initialDelaySeconds (live/ready) | 60~90s / 30~60s | 0s (startupProbe 위임) | [03 §6.2](OPENTRAUM-INFRA-03-WORKLOAD.md), [06 §4](OPENTRAUM-INFRA-06-OPERATIONS.md) |
| terminationGracePeriodSeconds | 30s 기본 | 10s 또는 40s (서비스별) | [03 §6.2](OPENTRAUM-INFRA-03-WORKLOAD.md), [06 §4](OPENTRAUM-INFRA-06-OPERATIONS.md) |

### Pod 리소스 (Spring 7 서비스 기준, monitoring 포함)

| 항목 | 튜닝 전 | 튜닝 후 | 출처 |
|---|---|---|---|
| memory requests | 128Mi | 512Mi (request=limit, idle+30% 헤드룸) | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md), [06 §9.1](OPENTRAUM-INFRA-06-OPERATIONS.md) |
| memory limits | 256Mi | 512Mi (request 와 동일, Guaranteed QoS) | 동일 |
| cpu requests | 250m | 차등 200~350m (auth 300m / gateway 300m / user 350m / event·payment·reservation·monitoring 200m) | 동일 |
| cpu limits | 500m | request 와 동일 (Guaranteed QoS) | 동일 |
| event-service mem limits | 768Mi | 512Mi (다른 백엔드와 통일) | [03 §4](OPENTRAUM-INFRA-03-WORKLOAD.md), [06 §9.1](OPENTRAUM-INFRA-06-OPERATIONS.md) |
| JVM 컨테이너 기동 시간 | JAVA_OPTS 미적용 또는 부분 적용 | TieredStopAtLevel=1 적용 (약 20% 단축, 추정) | [03 §5](OPENTRAUM-INFRA-03-WORKLOAD.md) |
...
flowchart LR
    ...
    subgraph EKS [EKS <CLUSTER_NAME> - ap-northeast-2]
        web[web<br/>opentraum ns<br/>replicas=1]
        gw[gateway<br/>opentraum ns<br/>replicas=1]
        auth[auth-service]
...
| opentraum | [03 WORKLOAD](OPENTRAUM-INFRA-03-WORKLOAD.md) | gateway, web, 6 도메인 백엔드 (auth/user/event/payment/reservation/monitoring) |
...
- 트래픽 테스트 시 KEDA 또는 HPA 재도입 예정. reservation-service 의 Kafka lag 기반 스케일이 1순위 후보입니다.
