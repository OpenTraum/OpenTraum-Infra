...
본 시스템의 CDC 도구는 [2장](#2-도구-스택과-버전) 의 표에 적힌 대로 Debezium MariaDB Connector 3.2.0 입니다. Strimzi KafkaConnect CR 의 build 정의에 connector tar 와 debezium-scripting 을 쌓아 Harbor 에 push 한 커스텀 이미지(`<HARBOR_REGISTRY>/<HARBOR_PROJECT>/team8-opentraum-debezium-connect:3.2.0`) 위에서 동작합니다. Debezium 은 binlog 의 변경 중에서도 우리가 지정한 테이블 변경만 읽도록 `table.include.list` 로 좁혀져 있어, outbox 테이블 외 비즈니스 테이블의 변경은 큐로 새지 않습니다.
...
KafkaConnect 이미지는 Strimzi build 기능으로 빌드된 커스텀 이미지(`<HARBOR_REGISTRY>/<HARBOR_PROJECT>/team8-opentraum-debezium-connect:3.2.0`) 입니다. Strimzi Operator 가 KafkaConnect CR 의 `spec.build` 정의를 보고 베이스 이미지 위에 Debezium MariaDB Connector tar 와 debezium-scripting 을 쌓아 Harbor 에 push 한 결과물이며, push 자격증명은 `harbor-registry-secret` 으로 관리됩니다.
...
`config.providers: secrets` + `KubernetesSecretConfigProvider` 가 설정되어 있어 KafkaConnector CR 의 일부 값을 Secret 참조 형태로 작성할 수 있는 통로가 열려 있습니다(현재는 라이브 connector CR 이 평문 값을 쓰고 있고, DB 자격증명은 Secret 으로 관리되는 별도 자격증명이 매니페스트에 들어가 있습니다).
...
| `database.user` | `opentraum` | GRANT REPLICATION SLAVE/CLIENT 보유 |
| `database.password` | DB 자격증명 (Secret 으로 관리) | 평문이 매니페스트에 들어가 있어 Secret 화 권장 |
| `table.include.list` | `{db}.outbox_events` | 도메인 outbox 테이블만 추적 |
| `snapshot.mode` | `initial` | connector 첫 기동 시 outbox 전체 스냅샷, 이후 binlog 따라가기 |
