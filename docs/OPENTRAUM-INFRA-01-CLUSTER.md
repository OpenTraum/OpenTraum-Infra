...
현재 워커 노드는 7개이며, AZ 2a 에 4개, AZ 2b 에 3개로 비대칭 배치되어 있습니다. 모두 ON_DEMAND 인스턴스이고, 6개는 t3.medium, 1개는 g5.xlarge 입니다.
...
t3.medium 6개 노드는 vCPU 2, 메모리 4GiB(allocatable ~3.2Gi) 의 burstable 인스턴스로, 평소에는 baseline 성능을 쓰다가 짧은 부하 구간에서 CPU credit 을 소모해 turbo 로 올라가는 구조입니다. 학습/데모 환경처럼 트래픽이 항상 높지 않고 비용 민감도가 큰 워크로드에 유리하므로, 백엔드 / 프론트 / 데이터(MariaDB, Redis, Kafka) / 관측(Prometheus, Loki, Tempo) 컴포넌트의 기본 거주지로 활용됩니다.

g5.xlarge 1개는 NVIDIA A10G GPU(24GB) 가 한 장 붙은 인스턴스로, vCPU 4, 메모리 16GiB(allocatable ~14.4Gi) 입니다. 라이브 점검 시점 기준으로 이 노드에는 시스템 데몬셋 이외의 워크로드가 거주하지 않으며, ML 추론/학습 등 GPU 가 필요한 워크로드를 올릴 때 활용할 후보 자원입니다. 일반 Pod 가 이 노드로 빨려 들어가지 않도록 향후 NodeSelector / Taint 를 함께 쓰는 것이 권장됩니다.
...
`gp2` 는 옛 in-tree provisioner(`kubernetes.io/aws-ebs`) 를 쓰는 legacy StorageClass 입니다. 새 PVC 에는 권장되지 않으며, 본 클러스터의 PVC 7개 중 어느 것도 gp2 를 사용하지 않습니다.
...
| data-tempo-0 | monitoring | 10Gi | RWO | ebs-sc | tempo-0 |
| storage-loki-0 | monitoring | 10Gi | RWO | ebs-sc | loki-0 |

총 영구 스토리지 용량은 50Gi 이며, 모두 `ebs-sc` 를 사용합니다. accessMode 는 전부 RWO(ReadWriteOnce) 로, 한 노드의 한 Pod 만 동시에 mount 가 가능한 EBS 의 특성을 그대로 반영합니다.

모든 PVC 가 EBS 인 이유는 위 표의 워크로드 특성 때문입니다. Kafka 브로커, MariaDB, 이벤트/예약/결제용 DB, Tempo(트레이스 저장), Loki(로그 인덱스/청크) 는 모두 단일 Pod 가 자기 데이터 디렉터리를 독점해서 쓰는 형태이며, 다중 노드에서 동시에 같은 파일을 쓰는 요구가 없으므로 RWO + AZ 종속 EBS 로 충분합니다. 그 대신 트레이드오프가 따라옵니다. 해당 Pod 는 PVC 가 묶인 AZ 의 노드로만 다시 스케줄될 수 있으므로, 그 AZ 에 가용 노드가 사라지면 Pod 가 Pending 으로 멈춥니다. EFS(`efs-sc`) 는 RWX 가 가능하지만 현재 그 요구사항이 없어 미사용 상태이고, `gp2` 는 in-tree legacy provisioner 라 신규 PVC 에는 권장하지 않습니다.
...
flowchart LR
  ...
  subgraph MON["ns: monitoring"]
    P6["data-tempo-0<br/>10Gi RWO"]:::obs
  ...
  P1 -. "owner: kafka-pool-0" .-> NODES["워커 노드 7개<br/>(AZ 2a 4 / AZ 2b 3)<br/>EBS 는 owner Pod 가 처음 스케줄된 AZ 에 고정"]:::worker
  P2 -. "owner: event-db-0" .-> NODES
  P3 -. "owner: payment-db-0" .-> NODES
  P4 -. "owner: reservation-db-0" .-> NODES
  P5 -. "owner: opentraum-mariadb-0" .-> NODES
  P6 -. "owner: tempo-0" .-> NODES
...
총 vCPU 14 코어 (g5.xlarge 4 + t3.medium 6 × 2). 총 allocatable 메모리는 약 33Gi (g5.xlarge ~14.4Gi + t3.medium 6 × ~3.2Gi). 영구 스토리지는 EBS 50Gi 한 종류로 통일되어 있고 모두 ebs-sc 를 통해 동적 프로비저닝됐습니다.
...
7개 노드가 전부 ON_DEMAND 단일 정책인 점은 비용/안정성 트레이드오프입니다. 예측 가능한 비용과 중단 없는 가용성을 얻는 대신, 스팟 인스턴스를 섞어 쓸 때 얻을 수 있는 70% 안팎의 비용 절감 여지를 포기한 상태입니다. 본 클러스터는 SKALA 학습 환경이므로 단순함이 우선이지만, 운영 환경으로 옮긴다면 stateless 워커(프론트, BFF, 웹훅 처리기) 만 별도 노드그룹으로 빼서 스팟에 올리는 패턴이 일반적입니다.
