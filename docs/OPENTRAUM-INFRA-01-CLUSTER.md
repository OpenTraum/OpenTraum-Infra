...
t3.medium 6개 노드는 vCPU 2, 메모리 4GiB(allocatable ~3.2Gi) 의 burstable 인스턴스로, 평소에는 baseline 성능을 쓰다가 짧은 부하 구간에서 CPU credit 을 소모해 turbo 로 올라가는 구조입니다. 학습/데모 환경처럼 트래픽이 항상 높지 않고 비용 민감도가 큰 워크로드에 유리하므로, 백엔드 / 프론트 / 데이터(MariaDB, Redis, Kafka) / 모니터링(Prometheus, Loki) 컴포넌트의 기본 거주지로 활용됩니다.
