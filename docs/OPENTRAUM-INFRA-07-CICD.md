...
> 이전: [06 OPERATIONS](OPENTRAUM-INFRA-06-OPERATIONS.md) · 다음: [08 TRAFFIC-TEST](OPENTRAUM-INFRA-08-TRAFFIC-TEST.md)

## 목차
- [1. 개요](#1-개요)
- [2. 한눈에 보는 전체 흐름](#2-한눈에-보는-전체-흐름)
- [3. 도구 스택과 모든 버전](#3-도구-스택과-모든-버전)
- [4. GitHub Repos 9개 카탈로그](#4-github-repos-9개-카탈로그)
- [5. 8 서비스 레포 cicd.yml 상세](#5-8-서비스-레포-cicdyml-상세)
...
- [9. ECR 이미지 레지스트리](#9-ecr-이미지-레지스트리)
- [10. ArgoCD GitOps](#10-argocd-gitops)
- [11. 무한루프 방지 정책](#11-무한루프-방지-정책)
...
본 장은 OpenTraum 시스템의 CI/CD 파이프라인 전 구간을 다룹니다. 다루는 범위는 개발자가 GitHub 레포지토리에 코드를 push 하는 순간부터, GitHub Actions 가 컨테이너 이미지를 빌드해 ECR 레지스트리에 push 하고, Kubernetes 매니페스트의 이미지 태그를 갱신하며, ArgoCD 가 변경을 감지해 EKS 클러스터에 RollingUpdate 를 적용하기까지의 한 사이클 전체입니다. 즉 코드 한 줄이 운영 클러스터에 반영되기까지의 전 자동화 경로를 한 장으로 정리합니다.

정보 출처는 다음 세 가지로 한정했습니다. 첫째, 9개 GitHub 레포지토리(`OpenTraum-Infra`, `OpenTraum-Web`, `OpenTraum-gateway`, `OpenTraum-auth-service`, `OpenTraum-user-service`, `OpenTraum-event-service`, `OpenTraum-reservation-service`, `OpenTraum-payment-service`, `OpenTraum-monitoring-service`) 의 `.github/workflows/` 디렉토리에 실재하는 워크플로 파일입니다. 둘째, 각 서비스 레포지토리 root 의 `Dockerfile` 입니다. 셋째, 라이브 EKS 클러스터의 `argocd` 네임스페이스에 떠 있는 ArgoCD 구성 요소(확인 시점 2026-04-28 15:10 KST)입니다. 본문에서 언급하는 모든 사실은 이 세 가지 출처 중 하나에 직접 대응됩니다.
...
본 장의 후반부에서는 자동화 과정에서 실제로 직면했던 문제(GitHub Actions 무한루프, ecr-secret 누락, kubectl client-side validation 충돌)와 그에 대한 결정 과정을 기록합니다. 코드 사실 외에 운영 결정 한 건이 본문에 정리되어 있습니다.

---

## 2. 한눈에 보는 전체 흐름

전체 사이클은 8개의 노드와 7개의 단계로 표현됩니다. 개발자가 PR 을 열면 Build Validation 워크플로(`build.yml`)가 단위 테스트를 돌려 머지 가능 여부를 보고합니다. PR 이 main 에 머지되면 `cicd.yml` 의 `build_image` 잡이 트리거되어 QEMU 와 Buildx 로 멀티 아키텍처 이미지를 빌드한 뒤 ECR 레지스트리(`881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>`) 에 push 합니다. 이미지 push 가 끝나면 같은 워크플로의 `update_manifest` 잡이 sed 로 `k8s/deployment.yml` 의 image 태그를 새로운 SHA 로 치환하고 main 브랜치에 커밋합니다. 이 매니페스트 변경을 ArgoCD `repo-server` 가 폴링으로 감지하고, `application-controller` 가 desired state 와 live state 를 비교해 차이가 있으면 `kube-apiserver` 에 Apply 를 호출합니다. 마지막으로 EKS 워커 노드의 kubelet 이 새 이미지를 ECR 에서 pull 받아 RollingUpdate 로 Pod 를 교체합니다.
...
flowchart LR
  ...
  classDef ecr fill:#D9D9D9,stroke:#444,color:#000;
  ...
  ECR["ECR<br/>881490135253.dkr.ecr.ap-northeast-2.amazonaws.com<br/><ECR_REPOSITORY_PREFIX>*"]:::ecr
...
이 한 장 안에서 두 가지 사이클이 분리되어 있다는 점이 핵심입니다. 좌측 절반(Dev → CICD → ECR) 은 CI 영역으로 이미지 산출물을 생성합니다. 우측 절반(GitMan → Argo → Pod) 은 CD 영역으로 산출물을 클러스터에 반영합니다. 두 영역의 접점은 단 하나, `k8s/deployment.yml` 의 image 태그 한 줄입니다. 이 분리가 11장에서 설명할 무한루프 방지 설계의 출발점이기도 합니다.
...
| Action | actions/setup-java | v4 | JDK 설치, Spring 7서비스 build.yml 에서 사용 |
...
| Action | aws-actions/amazon-ecr-login | v2 | ECR 레지스트리 로그인(임시 토큰 발급) |
| Action | docker/build-push-action | v5 | 멀티 아키텍처 build + push, GHA cache 백엔드 |
| Action | aws-actions/configure-aws-credentials | v4 | ECR 로그인 및 create-secret.yml 의 EKS 접근용 AWS 자격 |
| CLI | AWS CLI | runner 기본 제공 | `aws eks update-kubeconfig` 로 kubeconfig 발급, `aws ecr get-login-password` 로 토큰 발급 |
| Build | Gradle | 8.12 | Spring 7서비스 빌드 도구 |
...
| Registry | Amazon ECR | 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com | AWS 관리형 컨테이너 레지스트리 |
...
docker/build-push-action@v5 는 platforms 파라미터에 `linux/arm64,linux/amd64` 를 받아 단일 명령으로 두 아키텍처 이미지를 동시에 빌드합니다. Buildx 가 manifest list 를 자동 생성해 ECR 에 단일 태그로 push 하고, kubelet 은 노드 아키텍처에 맞는 이미지를 자동 선택해 pull 합니다. 이 multi-arch 정책은 12장 정량 성과 표의 출발점입니다.

ArgoCD 는 v3.3.6 이며, `argocd` 네임스페이스의 6개 핵심 컴포넌트(application-controller, applicationset-controller, notifications-controller, redis, repo-server, server) 가 모두 동일한 이미지(`quay.io/argoproj/argocd:v3.3.6`, redis 는 별도 이미지) 로 실행됩니다. 6개 컴포넌트 모두 `opentraum-best-effort` PriorityClass(value -100) 를 부여받아 노드 자원 압박 상황에서 가장 먼저 evict 되도록 설정되어 있습니다. Redis 는 컴포넌트 간 상태 캐시로 쓰이며, SSO 진입점인 Dex 는 현 클러스터에 배포되어 있지 않습니다(SSO 비활성, 운영자는 ArgoCD 로컬 admin 계정으로 접근).

---

## 4. GitHub Repos 9개 카탈로그

OpenTraum 코드베이스는 9개의 GitHub 레포지토리로 분리되어 있습니다. 인프라/매니페스트/도커 컴포즈를 담은 OpenTraum-Infra 1개, 사용자 진입 SPA 인 OpenTraum-Web 1개, Spring Cloud Gateway 1개, 그리고 Spring Boot 도메인 백엔드 6개(auth/user/event/reservation/payment/monitoring)입니다. 이렇게 분리한 이유는 첫째 도메인별 독립 배포 사이클을 보장하기 위함이고, 둘째 ArgoCD Application 의 단위와 Git 레포지토리 단위를 1:1 로 일치시켜 변경 추적을 단순화하기 위함입니다.

| # | 레포 | 언어/스택 | 빌드 워크플로 | 배포 워크플로 | ArgoCD App |
|---|---|---|---|---|---|
| 1 | OpenTraum/OpenTraum-Infra | Helm chart, k8s 매니페스트(`manual/k8s/`), docker-compose(`manual/local/`) | 없음 | create-secret.yml | (App-of-Apps 컨테이너) |
...
| 9 | OpenTraum/OpenTraum-monitoring-service | Spring Boot 21 | build.yml (gradle test) | cicd.yml | opentraum-monitoring-service |

`OpenTraum-Infra` 만 패턴이 다릅니다. 이 레포는 빌드 산출물을 만들지 않고, 클러스터 운영에 필요한 부속 자동화(ecr-secret 자동 생성)와 docker-compose 용 보조 Dockerfile 만 보관합니다. 또한 매니페스트는 각 서비스 레포 안의 `k8s/` 디렉토리에 있으므로(서비스 단위 GitOps), Infra 레포는 ArgoCD Application 의 직접 source 가 아닙니다.

`opentraum-web` 만 ArgoCD 등록 시 `git@github.com:OpenTraum/OpenTraum-Web.git` 형식으로 SSH 주소가 사용되어 있고, `opentraum-reservation-service` 만 `https://github.com/...` HTTPS 주소로 등록되어 있습니다. 나머지 6개는 SSH 주소 형식입니다. 양쪽 모두 동작하지만 ArgoCD 의 `repo-server` Pod 가 두 가지 인증(SSH 키 secret, HTTPS 토큰 secret) 을 모두 보유해야 한다는 점만 11장에서 다시 다룹니다.

---

## 5. 8 서비스 레포 cicd.yml 상세

본 절은 8개 서비스 레포(Web 1개, Gateway 1개, Spring 백엔드 6개) 가 공통으로 가진 `cicd.yml` 을 한 단위로 분석합니다. 모든 서비스가 동일한 골격을 사용하며 차이는 image 태그 이름, sed 패턴 매칭 키워드, git remote URL 의 레포 이름 부분 세 곳뿐입니다. 본 절에서는 OpenTraum-gateway 의 `cicd.yml` 을 대표 예제로 두고 라인 단위로 의도를 풀어쓴 뒤, 다른 서비스와의 차이를 마지막에 정리합니다.
...
| `jobs.build_image` | ECR 에 컨테이너 이미지를 빌드/푸시하는 첫 번째 잡 |
| `jobs.update_manifest.needs: build_image` | 이미지 푸시 성공 후에만 매니페스트 갱신 잡 실행 |

`permissions.contents: write` 라인이 핵심입니다. GitHub 는 2023년부터 워크플로의 GITHUB_TOKEN 기본 권한을 read 로 좁혔기 때문에, 매니페스트 commit/push 를 하려면 명시적으로 write 를 부여해야 합니다. 이 권한 명시는 CI 가 자기 레포에 push 할 수 있게 하는 최소 권한이며, 다른 레포에는 닿지 않습니다. 이 격리가 11장에서 다룰 무한루프 방지 정책의 전제입니다.

### 5.2 build_image 잡 5 step 분석

build_image 잡은 ubuntu-latest 러너 위에서 다섯 단계를 순차 실행합니다. 단계 사이의 인과는 이전 단계의 산출물(체크아웃된 소스, 활성화된 Buildx 빌더, ECR 로그인 세션) 이 다음 단계의 전제가 된다는 점에서 직선적입니다.
...
`fetch-depth: 0` 은 전체 git 히스토리를 가져옵니다. build_image 잡에서는 단일 SHA 빌드만 필요하므로 0 이 필수는 아니지만, 같은 워크플로 안에서 update_manifest 잡과 동작 일관성을 맞추기 위해 두 잡 모두 0 으로 통일했습니다. `token` 에는 GITHUB_TOKEN 이 명시적으로 전달됩니다. checkout 액션의 기본값으로 GITHUB_TOKEN 이 들어가지만, 명시는 11장 정책 결정의 흔적입니다.
...
#### step 4: ECR 로그인 (aws-actions/configure-aws-credentials@v4 + amazon-ecr-login@v2)
...
ECR 인증은 Harbor 식 username/password 가 아니라 AWS 자격증명으로부터 임시 토큰을 발급받는 방식입니다. configure-aws-credentials 가 러너에 AWS 자격증명을 주입하면 amazon-ecr-login 액션이 `aws ecr get-login-password` 를 호출해 12시간 유효한 임시 docker 토큰을 받아 docker config(`~/.docker/config.json`) 에 등록합니다. 같은 잡 안의 다음 step(build-push-action) 이 이 config 를 읽어 push 할 때 사용하며, 토큰은 워크플로 종료와 함께 폐기됩니다. 정적 docker 비밀번호가 GitHub Secrets 에 머무르지 않아 회전 부담이 없습니다(14장).
...
- uses: docker/build-push-action@v5
  with:
    ...
    tags: 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-gateway:${{ github.sha }}
    cache-from: type=gha
...
`platforms: linux/arm64,linux/amd64` 는 단일 호출로 두 아키텍처 이미지를 모두 빌드하라는 지시입니다. Buildx 는 두 아키텍처 빌드를 병렬로 실행한 뒤 manifest list 로 묶어 ECR 에 단일 태그로 push 합니다. 결과적으로 `opentraum-gateway:abc123` 이라는 한 태그 아래 arm64 이미지와 amd64 이미지가 모두 들어가 있고, EKS 노드의 kubelet 은 자기 노드 아키텍처에 맞는 쪽만 pull 받습니다. EKS 클러스터의 현 노드 풀은 amd64(`t3.medium` 6대 + `g5.xlarge` 1대, 총 7대) 로만 구성되어 있어 실질적으로 amd64 manifest 만 사용되고 있지만, arm64 이미지도 함께 push 해 두면 향후 Graviton 인스턴스로 일부 노드를 교체할 때 워크플로 변경 없이 그대로 동작합니다.

`tags` 에 `${{ github.sha }}` 가 들어가 있습니다. main 의 모든 커밋이 고유한 이미지 태그를 갖는 immutable tag 정책입니다. `latest` 같은 가변 태그는 사용하지 않으며, 이 정책 덕분에 `kubectl describe` 출력의 image 필드를 보면 어떤 commit 이 배포되었는지 즉시 확인할 수 있습니다. 또한 같은 태그가 두 번 push 되지 않으므로 ECR 에 캐시된 layer 가 의도치 않게 덮어써질 위험이 없습니다.
...
이 두 줄은 매니페스트 갱신 커밋의 author 가 사람이 아닌 GitHub Actions bot 임을 git 히스토리에 기록합니다. `41898282` 는 GitHub 가 모든 actions[bot] 에게 부여하는 고유 user ID 이고, `users.noreply.github.com` 도메인은 실제 이메일 주소가 아닌 GitHub 가 발급한 가상 주소입니다. 이 식별자가 11장의 후보 1(actor 체크) 에서 등장한 `github-actions[bot]` 이름의 출처입니다.
...
이 줄은 8개 서비스 cicd.yml 중 7개에 있고, OpenTraum-Web 의 cicd.yml 에는 누락되어 있습니다. 즉 Web 만 actions/checkout 이 자동 설정한 origin URL 에 의존합니다. 라이브 클러스터 사실상 Web 의 매니페스트 commit 이 정상 동작 중이므로(이미지 태그가 SHA 기반으로 갱신되어 있는 점은 03 WORKLOAD 에 기록) checkout 의 자동 설정으로 충분히 동작하고 있다고 추정되지만, 이는 GitHub 의 기본 동작에 의존한 결과이고 명시 설정이 없으므로 향후 GitHub 가 동작을 바꾸면 깨질 수 있는 잠재 위험입니다. 운영 측면에서는 다른 7개와 통일하는 후속 보강이 권장됩니다(15장 트러블슈팅 6번).

#### 명령 4: 매니페스트 패치 (sed)

```bash
sed -i "s|image: .*opentraum-gateway.*|image: 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-gateway:${{ github.sha }}|" k8s/deployment.yml
```
...
커밋 메시지에 새 SHA 가 그대로 박혀 들어가 git log 만으로 추적이 가능합니다. 운영 경험에 따라 `[skip ci]` 토큰은 제거되었습니다(11장). push 가 성공하면 ArgoCD 의 다음 폴링 주기에 변경이 감지되며, GitHub 측에서는 이 push 가 GITHUB_TOKEN 으로 발생했기 때문에 cicd.yml 워크플로의 재트리거가 플랫폼 레벨에서 자동 차단됩니다.

### 5.4 build.yml (PR 검증) 분석

build.yml 은 PR 단계에서 머지 가능 여부를 검증합니다. main 에 닿지 않는 가벼운 검증 잡이며, 두 변종이 존재합니다.

#### Spring 7서비스 build.yml
...
### 5.5 8 서비스 cicd.yml 의 차이점 정리

8개 cicd.yml 을 행 단위로 비교했을 때 의미 있는 차이는 다음과 같습니다.

| 차이 | 6 서비스 (auth, user, event, reservation, payment, monitoring) | gateway | web |
|---|---|---|---|
...
OpenTraum-Infra 레포의 단일 워크플로(`/.github/workflows/create-secret.yml`) 는 클러스터 운영 자동화의 첫 부속입니다. 목적은 단순합니다. `opentraum` 네임스페이스가 새로 만들어지거나 ecr-secret 이 삭제되었을 때, 손으로 `kubectl create secret docker-registry` 명령을 다시 치지 않도록 자동화하는 것입니다.
...
- name: ecr-secret 생성
  run: |
    ...
    kubectl create secret docker-registry ecr-secret \
      ...
      --docker-server=881490135253.dkr.ecr.ap-northeast-2.amazonaws.com \
      --docker-username=AWS \
...
ecr-secret 이 누락되면 어떤 일이 벌어지는지를 짚어 두면 자동화의 가치가 분명해집니다. opentraum 네임스페이스의 모든 Deployment 매니페스트(03 WORKLOAD) 는 `imagePullSecrets: [{name: ecr-secret}]` 를 포함하고 있고, kubelet 은 Pod 가 새 노드에 할당될 때마다 이 secret 으로 ECR 에 인증해 이미지를 pull 합니다. secret 이 없으면 kubelet 은 `ImagePullBackOff` 상태로 Pod 를 무한 재시도합니다. 클러스터 재구축이나 ns 재생성 직후, 또는 secret 만 실수로 삭제된 경우 즉시 모든 Pod 가 동일한 증상으로 멈춥니다.

ECR 토큰은 12시간만 유효하므로 secret 도 주기적으로 갱신되어야 합니다. 운영자가 토큰 만료를 매번 셸에서 처리하려면 `aws ecr get-login-password` 결과를 손으로 secret 에 옮겨야 하고, 그 과정에서 토큰이 셸 히스토리에 남는 위험이 있습니다. 자동화 워크플로가 AWS 자격증명을 GitHub Secrets 에서 읽어 토큰을 단발성으로 발급/적용하면 사람이 토큰을 만질 일이 사라집니다. 또한 cancel-in-progress 동시성 가드 덕분에 두 사람이 동시에 dispatch 해도 안전합니다.

---

## 7. Dockerfile 분석

OpenTraum 의 운영 이미지는 두 종류의 Dockerfile 로 빌드됩니다. Spring 7서비스(gateway 포함) 는 동일한 3-stage Dockerfile 을 사용하고, Web 은 2-stage 변종을 사용합니다. 본 절에서는 두 Dockerfile 을 stage 별로 풀어쓴 뒤, 각 stage 가 이미지 용량과 빌드 시간에 어떻게 기여하는지 정리합니다.
...
`OpenTraum-Infra/manual/local/docker/Dockerfile.spring` 과 `Dockerfile.web` 은 로컬 개발 환경의 docker-compose 에서만 사용하는 별도 변종입니다. 운영 빌드는 본 절의 7.1, 7.2 만 사용하며, 본 매뉴얼의 다른 장에서 docker-compose 변종이 등장하면 별도 표시합니다.

### 7.4 실제 이미지 크기 측정 (라이브)

위의 3-stage Dockerfile (Spring 7서비스) 와 2-stage Dockerfile (Web) 이 만들어 낸 실제 운영 이미지의 크기를 라이브 클러스터 노드의 image cache 에서 측정한 결과는 다음과 같습니다 (확인 시점 2026-04-28 15:10 KST, `kubectl get nodes -o json` 의 `status.images[].sizeBytes` 기준, OCI 압축 크기).

| 이미지 | 크기 | 베이스 |
|---|---|---|
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-web` | 19 MiB | nginx:1.25-alpine + dist |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-gateway` | 135 MiB | bellsoft/liberica-openjre-alpine:21 + Spring Boot Layered JAR |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-user-service` | 136 MiB | (동일) |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-monitoring-service` | 138 MiB | (동일) |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-auth-service` | 144 MiB | (동일) |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-payment-service` | 181 MiB | (동일) |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-event-service` | 198 MiB | (동일) + OpenAI SDK |
| `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-reservation-service` | 207 MiB | (동일) + Redisson + Kafka 컨슈머 |

Spring 7서비스 평균은 약 163 MiB 이고, web 은 1자리수 MiB 입니다. Spring 서비스간 크기 차이는 도메인별 의존성(예: event 의 OpenAI SDK, reservation 의 Redisson 분산 락 라이브러리 + Kafka 클라이언트) 의 양에서 발생합니다. layered JAR 의 효과는 이 표의 절대 크기에는 보이지 않지만, 코드만 변경된 빌드의 push 단계에서 의존성 layer (이미지 크기의 가장 큰 비중) 가 캐시 hit 으로 재업로드되지 않는 형태로 나타납니다.

(참고) 같은 클러스터의 KafkaConnect 커스텀 이미지 `881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>debezium-connect:3.2.0` 는 Strimzi build 결과물로 약 398 MiB 입니다. 이는 Debezium MariaDB Connector tar (수십 MB) + Outbox EventRouter SMT 가 Strimzi 베이스 이미지 위에 쌓여 ECR 로 push 된 크기입니다.
...
flowchart LR
  ...
  subgraph BUILD["build_image job"]
    ...
    B4["4. configure-aws-credentials<br/>+ amazon-ecr-login"]:::ci
    B5["5. build-push-action<br/>linux/arm64+amd64<br/>cache: type=gha,mode=max<br/>tags: SHA"]:::ci
    B1 --> B2 --> B3 --> B4 --> B5
  end

  ECR["ECR<br/>881490135253.dkr.ecr.ap-northeast-2.amazonaws.com<br/><ECR_REPOSITORY_PREFIX>opentraum-*<br/>(manifest list)"]:::ecr
...
Amazon ECR 은 OpenTraum CI/CD 의 단일 컨테이너 레지스트리입니다. EKS 와 같은 AWS 계정/리전(ap-northeast-2) 에 위치한 관리형 레지스트리이며, 본 프로젝트는 `<ECR_REPOSITORY_PREFIX>` prefix 로 8개 이미지 저장소를 보유합니다.

| 항목 | 값 |
|---|---|
| Registry 호스트 | 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com |
| Repository prefix | <ECR_REPOSITORY_PREFIX> |
| 이미지 저장소 | opentraum-gateway, opentraum-auth-service, opentraum-user-service, opentraum-event-service, opentraum-reservation-service, opentraum-payment-service, opentraum-monitoring-service, opentraum-web |
| Tag 정책 | Git commit SHA (immutable) |
| Manifest 형식 | Manifest list (linux/arm64 + linux/amd64) |
| 인증 (CI 측) | GitHub Secrets 의 AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY 로 임시 토큰 발급 |
| 인증 (클러스터 측) | opentraum ns 의 Secret `ecr-secret` (type docker-registry, 12시간 토큰) |
...
flowchart LR
  ...
  GHA["GitHub Actions<br/>AWS_ACCESS_KEY_ID/SECRET<br/>(GitHub Secrets)"]:::ci
  Push["build-push-action<br/>multi-arch push"]:::ci
  ECR["ECR<br/>881490135253.dkr.ecr.ap-northeast-2.amazonaws.com<br/><ECR_REPOSITORY_PREFIX>opentraum-*"]:::ecr
  Secret["opentraum ns<br/>ecr-secret<br/>(docker-registry, 12h 토큰)"]:::sec
  Kubelet["EKS kubelet<br/>imagePullSecrets"]:::app
  Pod["Pod 생성<br/>linux/amd64 또는 arm64"]:::app

  GHA --> Push --> ECR
  Secret -.dockerconfig.-> Kubelet
  Kubelet --image pull--> ECR
  ECR --image layers--> Pod
```

CI 측은 GitHub Secrets 의 AWS 자격증명을 매 워크플로 실행 시점에만 사용해 ECR 임시 토큰을 발급받아 push 합니다. 클러스터 측은 create-secret.yml 워크플로가 미리 생성한 `ecr-secret` 을 imagePullSecrets 로 마운트해 pull 합니다. 양쪽이 같은 AWS 계정의 ECR 권한을 공유하지만, 한쪽 자격이 새서 다른 쪽 권한을 침해할 위험은 GitHub Secrets 와 Kubernetes Secret 의 격리된 저장 및 ECR 토큰의 12시간 자동 만료에 의해 분리됩니다.

### 9.2 ecr-secret 자동 생성 흐름과 `--validate=false` 사유

6장에서 다룬 `create-secret.yml` 이 이 secret 의 단일 출처입니다. 운영 절차는 다음과 같습니다.

1. opentraum ns 가 새로 만들어지거나 secret 이 망가졌다(또는 12시간 토큰이 만료되었다)고 감지되면 운영자가 GitHub UI 에서 `workflow_dispatch` 로 실행합니다.
2. 워크플로는 AWS 자격증명으로 ECR 임시 토큰을 발급받고 EKS API 에 자격증명으로 접근해 kubeconfig 를 받은 뒤, `kubectl create secret docker-registry ecr-secret --dry-run=client -o yaml | kubectl apply --validate=false -f -` 를 실행합니다.
...
### 10.1 App-of-Apps 패턴과 Application 8개

라이브 클러스터의 `argocd` 네임스페이스에는 8개 Application 이 등록되어 있고 모두 Synced/Healthy 상태입니다. 모두 단일 AppProject(`default`) 에 속합니다. App-of-Apps 패턴(부모 Application 이 자식 Application 들을 관리하는 패턴) 자체는 직접 등록되어 있지 않고, 8개 Application 이 1세대로 평탄하게 등록되어 있습니다. 이는 서비스 수가 8개로 한정적인 현 단계에서 추가 추상화가 필요 없다는 판단의 결과입니다.
...
| opentraum-monitoring-service | git@github.com:OpenTraum/OpenTraum-monitoring-service.git | k8s | opentraum |
...
8개 모두 source path 가 `k8s` 디렉토리로 통일되어 있습니다. 즉 각 서비스 레포의 `k8s/` 디렉토리가 ArgoCD 의 desired state 단일 출처(SSoT) 입니다. `k8s/` 안에는 deployment.yml, service.yml, networkpolicy.yml, hpa.yml 등이 들어가며, ArgoCD 는 디렉토리 안의 모든 yaml 을 재귀적으로 수집해 적용합니다.

`opentraum-reservation-service` 만 HTTPS URL 로 등록되어 있고 나머지는 SSH URL 입니다. 이 차이는 ArgoCD repo-server 가 두 가지 인증 방식(SSH key Secret, HTTPS token Secret) 을 모두 보유해야 함을 의미합니다. 라이브에서 8개 Application 모두 Synced 인 사실로 보아 양쪽 인증이 정상 동작 중이며, 이 차이는 운영상 위험 신호는 아닙니다.
...
라이브 클러스터의 `argocd` ns 에는 6개 Pod 가 떠 있습니다. 각자 역할이 분리되어 있고, Redis 가 컴포넌트 간 캐시 채널 역할을 합니다. SSO 진입점인 Dex 는 현 시점 배포되어 있지 않습니다.
...
flowchart LR
  ...
  Git["GitHub Repos<br/>(8 서비스)"]:::external
...
6개 컴포넌트(application-controller, applicationset-controller, notifications-controller, redis, repo-server, server) 는 모두 `opentraum-best-effort` PriorityClass(value -100) 를 부여받아 실행됩니다. 이는 ArgoCD 가 운영 트래픽을 직접 처리하는 컴포넌트가 아닌 controller 성격이라는 점에서, 노드 자원 압박 상황에서 운영 워크로드(Spring 7서비스, web) 보다 먼저 evict 되어도 비즈니스 영향이 작다는 판단을 반영합니다. evict 후 재기동되어도 Git 이 SSoT 이므로 선언 상태가 그대로 복원됩니다.
...
argocd-applicationset-controller 는 ApplicationSet CRD(여러 Application 을 템플릿으로 일괄 생성하는 메커니즘) 를 처리합니다. 현 단계에서 ApplicationSet 은 사용하지 않습니다(8개 Application 이 직접 등록되어 있음). 향후 서비스 수가 늘어 동일 패턴 Application 을 자동 생성해야 한다면 이 컨트롤러를 활용합니다.
...
ArgoCD 의 SSO 기능은 별도 Dex 컴포넌트(또는 `argocd-cm` 의 OIDC 직접 설정) 가 배포되어야 활성화되지만, 현 클러스터에는 Dex 가 배포되어 있지 않으며 운영자는 ArgoCD 로컬 admin 계정으로 접근합니다. 향후 SSO 가 필요하다면 Dex 를 추가 배포하거나 OIDC 를 직접 연결합니다.

`argocd` ns 에는 또한 컴포넌트별 NetworkPolicy 가 적용되어 있어 컴포넌트 간 통신 경로가 좁혀져 있습니다(상세는 [02 NETWORK](OPENTRAUM-INFRA-02-NETWORK.md)).
...
`update_manifest` 잡이 main 에 매니페스트 commit 을 push 하는 순간, 그 push 가 다시 cicd.yml 을 트리거합니다. 트리거된 워크플로는 또 이미지를 빌드하고 다시 매니페스트를 commit 하며, 이 사이클이 무한히 반복됩니다. 결과적으로 GitHub Actions 분이 무한 소비되고 ECR 에 같은 코드의 이미지가 SHA 만 달리하며 누적되며, ArgoCD 도 매번 sync 를 수행합니다. 이 사슬을 어딘가에서 끊어야 합니다.
...
초기 정책 결정은 후보 2 + 후보 3 + 후보 4 의 3중 방어를 권장으로 적었지만, 그 이후 운영 경험을 거쳐 후보 4(`[skip ci]`) 는 제거되었습니다. 현재 라이브 cicd.yml 의 commit 메시지(`chore: 이미지 태그 업데이트 [SHA]`) 에는 `[skip ci]` 토큰이 없습니다. 즉 현 시점의 운영 모드는 다음 두 층입니다.
...
2. **2차 (보조): paths-ignore.** OpenTraum-Infra 의 `create-secret.yml` 만 paths 필터(`'.github/workflows/create-secret.yml'`) 를 사용하며, 8 서비스 cicd.yml 에는 paths-ignore 가 적용되어 있지 않습니다. 8 서비스의 경우 GITHUB_TOKEN 단독으로 충분하다는 판단입니다.
...
| 5. Config Repo 분리 | 8개 서비스 규모에서 레포 구조 변경은 과도. 향후 10+ 서비스 시 재검토 |
| 6. Image Updater | 일반 YAML 매니페스트 지원 제한, 프로덕션 비권장 명시(공식 문서) |

초기 검토는 향후 재검토 사항으로 두 가지를 적었습니다. 첫째, 서비스 수가 10개 이상으로 확장되면 Config Repo 분리(후보 5) 재검토. 둘째, Helm/Kustomize 로 전환하면 Image Updater(후보 6) 재검토. 두 조건 모두 현 단계에서는 충족되지 않습니다.
...
| ecr-secret 자동 생성 워크플로 | 운영자가 ECR 토큰을 셸에서 손으로 발급/적용하는 일 0. AWS 자격증명은 GitHub Secrets 에서 단발성으로 사용, 토큰은 12시간 자동 만료 | create-secret.yml |
| 멀티 아키텍처 + alpine 베이스 | 튜닝 전 별도 platform 빌드 워크플로 필요. 튜닝 후 단일 워크플로로 arm64 + amd64 manifest list 동시 산출. 운영 이미지 평균 크기 (Spring 7서비스 평균 163 MiB, web 19 MiB) 는 §7.4 표 참조 | Dockerfile 베이스 이미지 선택, §7.4 라이브 측정값 |
...
`imagePullPolicy: Always` 는 매 Pod 생성 시 kubelet 이 ECR 에 이미지 layer 의 변경 여부를 확인하게 합니다. SHA 기반 immutable tag 를 쓰면서 굳이 Always 인 이유는, 같은 SHA 가 두 번 push 될 일은 없지만 노드가 새로 추가될 때 캐시되지 않은 이미지가 즉시 pull 되도록 보장하기 위함입니다. 추가 비용은 manifest list 한 번 조회 정도로 무시 가능한 수준입니다.
...
| 변수명 | 사용처 | 비고 |
|---|---|---|
| AWS_ACCESS_KEY_ID | configure-aws-credentials | 8 서비스 cicd.yml(ECR 로그인용) + create-secret.yml(EKS 접근용) |
| AWS_SECRET_ACCESS_KEY | configure-aws-credentials | 위와 동일 |
| GITHUB_TOKEN | actions/checkout, git remote set-url | GitHub 가 워크플로마다 자동 발급/폐기. 별도 등록 불필요 |

GITHUB_TOKEN 은 운영자가 등록한 secret 이 아니라 GitHub 플랫폼이 매 워크플로 실행 시 자동 발급해 종료 시 자동 폐기하는 휘발성 토큰입니다. PAT 와 달리 회전이 필요 없고, 누출되어도 워크플로 종료와 동시에 무효화됩니다. 이 자동 회전이 11장의 GITHUB_TOKEN 채택 근거 중 하나입니다.
...
**증상.** GitHub Actions 분이 의도하지 않게 빠르게 소진되고, 같은 코드 base 에서 SHA 만 달라지는 이미지가 ECR 에 누적됩니다. cicd.yml 의 build_image 가 1~2분 간격으로 끝없이 트리거됩니다.
...
### 15.2 ImagePullBackOff (ecr-secret 누락 또는 토큰 만료)

**증상.** opentraum ns 의 신규/재생성된 Pod 가 모두 `ImagePullBackOff` 또는 `ErrImagePull` 상태에 들어갑니다. 기존에 떠 있던 Pod 는 정상.

**원인 가설.** ecr-secret 이 ns 에 존재하지 않거나, ECR 임시 토큰(12시간) 이 만료되었거나, secret 의 docker-server/username/password 가 오기재되었을 가능성. ns 가 새로 만들어진 직후이거나, 누군가 secret 을 실수로 삭제한 직후, 또는 12시간 이상 secret 갱신이 없었던 직후.

**진단.**

```bash
kubectl get secret -n opentraum ecr-secret -o yaml
kubectl describe pod -n opentraum {pod-name} | grep -A5 "Events:"
```

events 에 `Failed to pull image ... 401 Unauthorized` 또는 `secret \"ecr-secret\" not found` 가 보이면 확정됩니다.
...
**원인.** 5.5 절에서 적은 Web cicd.yml 의 명령 3 누락. 다른 7개 서비스에는 명시 라인이 있어 동작이 보장됩니다.
...
**증상.** Spring 7서비스 PR build.yml 의 첫 실행이 5~7분 소요되고, 이후 PR 은 1~2분으로 끝남.
...
**원인.** build-push-action 의 `platforms` 키가 누락되었거나 한 아키텍처만 지정됨. ECR 은 manifest list(OCI image index) 를 기본 지원하므로 레지스트리 측 호환성 문제는 발생하지 않습니다.

**진단.**

```bash
docker buildx imagetools inspect 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-gateway:{sha}
```

출력에 `Platform: linux/arm64` 와 `Platform: linux/amd64` 가 모두 보여야 합니다.

**복구.** 현 시점 cicd.yml 은 `platforms: linux/arm64,linux/amd64` 가 명시되어 있어 정상입니다. ECR 측 지원도 라이브에서 multi-arch push 가 동작 중인 것으로 확인됩니다.
...
# === ECR 인증 (클러스터 측) ===
# secret 존재 확인
kubectl get secret -n opentraum ecr-secret
...
# === ECR 이미지 검증 ===
# 임시 docker 토큰 발급(로컬에서 inspect 하려는 경우)
aws ecr get-login-password --region ap-northeast-2 | docker login --username AWS --password-stdin 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com

# manifest list 확인 (arm64 + amd64 모두 포함되었는지)
docker buildx imagetools inspect 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-gateway:{sha}

# 특정 이미지의 layer 크기 확인
docker buildx imagetools inspect 881490135253.dkr.ecr.ap-northeast-2.amazonaws.com/<ECR_REPOSITORY_PREFIX>opentraum-gateway:{sha} --raw
...
> 본 장의 모든 사실은 9개 GitHub 레포지토리의 워크플로 파일, 각 서비스 레포의 Dockerfile, 라이브 EKS 클러스터의 argocd 네임스페이스 상태(2026-04-28 15:10 KST) 의 3가지 출처에 직접 대응됩니다. 본문 외 항목(SealedSecret/SOPS 권고, Config Repo 분리, Image Updater 도입) 은 향후 검토 사항으로 명시되었으며 현 상태는 아닙니다.
